...
await act(async () => { payButton().props.onClick(); payButton().props.onClick(); });
assert.ok(treeText().includes('Processing…'));
assert.equal(payButton().props.disabled, true);
...
for (const outcome of ['decline', 'timeout']) {
  ...
  await act(async () => { emailInput().props.onChange({ target: { value: 'payer@example.com' } }); });
  await act(async () => { payButton().props.onClick(); });
  assert.equal(renderer.root.findByProps({ role: 'alert' }).props['data-payment-state'], outcome === 'decline' ? 'declined' : 'timeout');
