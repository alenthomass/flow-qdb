...
export class Component {
  ...
  pageVals(s, d) {
    ...
    const invoices = onInv ? s.invoices.map(i => Object.assign({}, i, { amt: this.fmt(i.amount), chip: this.chip(i.status), open: this.open('invoice', i.id) })) : [];
    const remindable = { Sent: 1, Viewed: 1, Overdue: 1, 'Awaiting Settlement': 1 };
    const reminderInvoices = onInv ? (s.reminderInvoices && s.reminderInvoices.length ? s.reminderInvoices : invoices.filter(i => remindable[i.status])).map(i => Object.assign({}, i, {
    ...
    return {
      ...
      counts: { rows: String(onTxn ? rows.length : s.txns.length), total: String(s.txns.length), invoices: String(s.invoices.length), matches: String(this.liveMatch().open) },
