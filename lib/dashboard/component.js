...
export class Component {
  ...
  plain(n) { return Math.abs(Number(n) || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  ...
  copyPayUrl(url, quiet) {
    ...
    if (clip && text) {
      Promise.resolve(navigator.clipboard.writeText(text)).then(() => {
        if (!quiet) this.toast('Link copied to clipboard');
      }).catch(() => {
        if (!quiet) this.toast('Link copied to clipboard');
      });
      return;
    }
    if (!quiet) this.toast(text ? 'Link copied to clipboard' : 'No link to copy');
  }
  ...
  componentDidMount() {
    ...
    this._onKey = (e) => {
      if ((e.metaKey || e.ctrlKey) && (e.key === 'k' || e.key === 'K')) {
        ...
        this.setState(s => (s.modal ? { modal: null } : s.invFilterOpen ? { invFilterOpen: null } : s.searchOpen ? { searchOpen: false, search: '' } : s.moreOpen ? { moreOpen: false } : s.userMenu ? { userMenu: false } : s.fly ? { fly: null, railOpen: false } : s.pickFor ? { pickFor: null } : s.detail ? { detail: null } : null));
      }
    };
    this._onPointer = (e) => {
      const t = e.target;
  ...
  pageVals(s, d) {
    ...
    const onPayr = this.needPage('payroll') || detType === 'employee';
    ...
    const team = onTeam ? s.team.map(m => Object.assign({}, m, { open: this.open('member', m.id), initials: this.initialsOf(m.name) })) : [];
    ...
    if (d) {
      ...
      if (det.invoice) {
        ...
        const lineRows = (i.lines && i.lines.length)
          ? i.lines.map(line => {
              ...
              return {
                ...
                qty: String(qty),
                unit: this.plain(unitMajor),
                amt: this.plain(majorAmt),
                note: line.note || ''
              };
            })
          : [{
              label: 'Invoice',
              qty: '1',
              unit: this.plain((Number(i.amount) || 0) + discMajor),
              amt: this.plain((Number(i.amount) || 0) + discMajor)
            }];
        ...
        det.o = Object.assign({}, i, {
          ...
          previewSubtotal: this.fmt(lineMajor),
          previewDiscount: discMajor > 0 ? this.fmt(discMajor) : '',
      ...
      if (det.member) { const m = find(s.team, d.id); det.o = Object.assign({}, m, { initials: this.initialsOf(m.name) }); }
      if (det.branch) {
    ...
    return {
      ...
      providers: onConn ? s.providers.map(p => Object.assign({}, p, { initial: p.n.slice(0, 2), note: p.s !== 'Coming soon', go: () => this.toast(p.n + ' is not available in Phase 1') })) : [],
      ...
      tagList: this.needPage('settings', 'tags') ? (s.ledgerTags && s.ledgerTags.length ? s.ledgerTags : s.tags).map(t => ({
        id: t,
        label: t,
      ...
      payroll: onPayr ? (() => {
        ...
        return {
          total: this.fmt0(gross), net: this.fmt0(net), count: String((s.employees || []).length), deductions: this.fmt0(ded), pctText: (pct * 100).toFixed(pct * 100 % 1 ? 1 : 0) + '%',
          period,
      ...
      })() : { total: '', net: '', count: '0', deductions: '', pctText: '', period: '', alreadyPosted: false, canPost: false, postLabel: '', postStyle: '', setPeriod: noop, post: noop },
      connStats: (() => {
  ...
  renderVals() {
    ...
    const vals = {
      ...
      nv: this.needPage('invoicing', 'create') ? (() => {
        ...
        const files = Array.isArray(iv.attachments) ? iv.attachments : [];
        ...
        const saveInvoice = (draft) => {
          ...
          const liveFiles = Array.isArray(live.attachments) ? live.attachments : [];
          ...
          const saved = this.persistCreatedInvoice({
            ...
            partialPayment: !!live.partialPayment,
            discountMinor: Math.round(liveDisc * 100),
            attachments: liveFiles,
        ...
        return {
          ...
          partialOn: !!iv.partialPayment,
          partialTrack: 'position:relative; width:42px; height:24px; flex:0 0 42px; border-radius:13px; padding:0; border:1px solid ' + (iv.partialPayment ? 'transparent' : 'var(--line)') + '; background:' + (iv.partialPayment ? 'linear-gradient(180deg,var(--ink-2),var(--ink-block))' : 'var(--toggle-off)'),
          partialKnob: 'position:absolute; top:2px; left:2px; width:18px; height:18px; border-radius:50%; background:#FFFFFF; box-shadow:0 1px 3px rgba(0,0,0,.3); transform:translateX(' + (iv.partialPayment ? '18px' : '0') + '); transition:transform .28s cubic-bezier(.32,.72,0,1)',
          togglePartial: () => patch('partialPayment', !iv.partialPayment),
          files: files.map((file, idx) => ({
            name: file.name,
            remove: () => patch('attachments', files.filter((_, j) => j !== idx))
          })),
          addFiles: (e) => {
            const added = Array.prototype.slice.call(e.target.files || []).map(f => ({ name: f.name, size: f.size || 0 })).filter(f => f.name);
            if (!added.length) return;
            patch('attachments', files.concat(added));
            e.target.value = '';
          },
      ...
      gatewayOptions: this.needPage('payments', 'gateway') ? [
      ...
      ppList: this.needPage('payments', 'checkout') ? {
        ...
        rows: (s.checkoutPages || s.ppPages || []).map(p => ({
          ...
          amount: p.amountText || (p.amountMode === 'open' ? 'Open' : (p.amount ? this.fmt0(p.amount) : '—')),
          open: () => this.setState({
      ...
      } : { show: s.ppView === 'list', edit: s.ppView === 'edit', published: s.ppView === 'published', count: '0 pages', empty: true, rows: [], create: noop, back: noop, reopen: noop },
      rcSummary: s.rc.auto
      ...
      results: (() => {
        ...
        if (cache && cache.q === q && cache.txns === s.txns && cache.invoices === s.invoices && cache.clients === s.clients) {
          return cache.output;
        }
        if (!q) {
          const output = { groups: [], none: false };
          this._searchResultsCache = { q, txns: s.txns, invoices: s.invoices, clients: s.clients, output };
          return output;
        }
      ...
      mobData: mob ? (() => {
        ...
        return {
          balance: period.netText || this.fmt(period.net), inflow: this.fmt0(inflow), outflow: this.fmt0(Math.abs(outflow)),
          recent,
          actions: [
            { label: 'Request', d: 'M10 4.4v11.2M5.6 11.2 10 15.6l4.4-4.4', go: this.openModal('link') },
            { label: 'Invoice', d: 'M6.1 2.8h7.8v14.4l-1.95-1.3-1.95 1.3-1.95-1.3L6.1 17.2ZM8.4 9.6h3.2', go: this.go('invoicing', 'create') },
      ...
      })() : { balance: '', inflow: '', outflow: '', recent: [], actions: [], needsMatch: '0', openInv: '0', outstanding: '', pending: '', delta: '', allTxns: [], invoices: [] },
      greet: (() => {
      ...
      bank: (typeof FlowStore !== 'undefined' && FlowStore.bankView) ? FlowStore.bankView() : (s.bank || {}),
      ...
      detail: { on: !!d },
      modal: { on: !!s.modal, title: md[0], cta: md[1], hideFoot: s.modal === 'link', wide: s.modal === 'link', link: s.modal === 'link', scan: s.modal === 'scan', expense: s.modal === 'expense', invoice: s.modal === 'invoice', plan: s.modal === 'plan', member: s.modal === 'member', employee: s.modal === 'employee', help: s.modal === 'help', pageSettings: s.modal === 'pageSettings', receipts: s.modal === 'receipts', reset: s.modal === 'reset' },
      ...
      recent: (() => {
        ...
        if (cache && cache.txns === txns) return cache.output;
        const output = this.sortedTxns().slice(0, 6).map(t => this.row(t));
        this._recentFeedCache = { txns, output };
        return output;
      })(),
      att: {
