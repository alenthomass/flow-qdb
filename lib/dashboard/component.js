...
export class Component {
  ...
  openModal(kind) {
    return this.memo('_md', kind, () => () => this.setState(s => {
      ...
      return { modal: kind, scanDone: false, scanBusy: false, scanLines: [], scanConf: {}, editEmp: null, editClient: null, editTag: null, form, invDueError: '' };
    }));
  }
  ...
  pageVals(s, d) {
    ...
    const syncLog = onAcc ? (s.syncLog || []).map(l => Object.assign({}, l, { chip: this.chip(l.status), open: this.open('synclog', l.id), itemsT: String(l.items), errT: String(l.errors) })) : [];
    ...
    return {
      ...
      bankOn: (() => {
        const extras = Array.isArray(s.bank && s.bank.extra) ? s.bank.extra : [];
        const linked = Array.isArray(s.banks) ? s.banks : [];
        const catalog = (Array.isArray(s.sampleBanks) && s.sampleBanks.length)
          ? s.sampleBanks
          : ((typeof FlowStore !== 'undefined' && FlowStore.SAMPLE_BANKS) || []);
        const remaining = catalog.filter(b => !extras.some(x => x.id === b.id) && !linked.some(x => x.id === b.id));
        const step = s.bankOnboarding || 0;
        const picked = remaining.find(b => b.id === s.bankPick) || remaining[0] || null;
        const remainingNow = () => {
          const st = this.state;
          const ex = Array.isArray(st.bank && st.bank.extra) ? st.bank.extra : [];
          const ln = Array.isArray(st.banks) ? st.banks : [];
          const cat = (Array.isArray(st.sampleBanks) && st.sampleBanks.length)
            ? st.sampleBanks
            : ((typeof FlowStore !== 'undefined' && FlowStore.SAMPLE_BANKS) || []);
          return cat.filter(b => !ex.some(x => x.id === b.id) && !ln.some(x => x.id === b.id));
        };
        return {
          idle: step === 0,
          pick: step === 1 && remaining.length > 1,
          consent: step === 2 || (step === 1 && remaining.length === 1),
          hasRemaining: remaining.length > 0,
          noneLeft: remaining.length === 0,
          picked: picked ? picked.label : '',
          start: () => {
            const left = remainingNow();
            if (!left.length) {
              this.toast('All sample banks in this sandbox are connected');
              this.setState({ bankOnboarding: 0, bankPick: null });
              return;
            }
            this.setState({
              bankOnboarding: left.length === 1 ? 2 : 1,
              bankPick: left[0].id
            });
          },
          choices: remaining.map(b => ({
            id: b.id,
            label: b.label,
          ...
          confirm: () => {
            const left = remainingNow();
            const id = this.state.bankPick || (left[0] && left[0].id);
            if (!id) {
              this.toast('Pick a sample bank first');
              return;
            }
            if (typeof FlowStore === 'undefined' || !FlowStore.connectSampleBank) {
              this.toast('Sample bank connect is not available');
              return;
            }
            try {
              FlowStore.connectSampleBank(id);
              this.applyStore({ bankOnboarding: 0, bankPick: null });
              this.toast('Sample bank connected');
            } catch (err) {
              this.toast((err && err.message) || 'Could not connect sample bank');
            }
          }
        };
      })(),
    ...
    const vals = {
      ...
      bank: (() => {
        ...
        return Object.assign({}, src, {
          ...
          note: src.note ? this.display(src.note) : src.note,
          extra: (src.extra || []).map(row => Object.assign({}, row, {
      ...
      detail: { on: !!d },
      modal: { on: !!s.modal, title: md[0], cta: md[1], hideFoot: s.modal === 'link', wide: s.modal === 'link', link: s.modal === 'link', scan: s.modal === 'scan', expense: s.modal === 'expense', invoice: s.modal === 'invoice', plan: s.modal === 'plan', member: s.modal === 'member', employee: s.modal === 'employee', client: s.modal === 'client', tag: s.modal === 'tag', help: s.modal === 'help', pageSettings: s.modal === 'pageSettings', receipts: s.modal === 'receipts', reset: s.modal === 'reset' },
