...
export class Component {
  ...
  pageVals(s, d) {
    ...
    const team = onTeam ? s.team.map(m => Object.assign({}, m, { open: this.open('member', m.id), initials: m.name.split(' ').map(x => x[0]).join('') })) : [];
        ...
        return {
          ...
          addClient: () => {
            ...
            if (typeof FlowStore !== 'undefined' && FlowStore.addClient) {
              FlowStore.addClient({ name: nm, address: String((s.nv && s.nv.clientAddress) || '').trim() });
              this.applyStore({
                nvClients: false,
          ...
          downloadPdf: () => {
            ...
            this.downloadCurrentInvoicePdf(nextNo, live.client);
          },
          printInvoice: () => {
            flushNvDrafts();
            const live = this.state.nv || {};
