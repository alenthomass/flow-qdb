...
import { downloadInvoicePdf, printInvoicePreview } from "./invoice-export";
import { flushNvDrafts } from "./nv-draft.js";
import { applyLocaleDir, createTranslator, enabledLocales, localizeDigits, localizeDisplay, readStoredLocale, storeLocale, translateStatus } from "../i18n/index.ts";

function toDateInput(raw, fallbackOffset) {
  if (!FlowStore || !FlowStore.dateInputValue) return raw || '';
  if (raw && FlowStore.offsetFromLabel) {
    const off = FlowStore.offsetFromLabel(String(raw));
    if (off != null) return FlowStore.dateInputValue(off);
  }
  if (Number.isFinite(fallbackOffset)) return FlowStore.dateInputValue(fallbackOffset);
  return raw || '';
}

const REPORTS_VALS_IDLE = {
  reports: [],
  reportsLabel: '',
  pnl: { periodLabel: '', net: '', netColor: 'var(--ink)', margin: '', cards: [], rows: [] },
  runway: { cash: '', burn: '', burnLabel: '', months: '', until: '' },
  forecast: [],
  spend: { tags: [], vendors: [], refunds: '', hasRefunds: false, insight: '' },
  ageing: { buckets: [], rows: [] },
  pack: { period: '', accountant: '', items: [], monthlyOn: false },
  branchPerf: [],
  aiInsights: []
};
...
const PP_VALS_IDLE = {
  ...
  tools: [],
  share: [],
  priceMenu: { options: [], open: false, label: '', toggle: function () {}, close: function () {} },
  url: '',
  headline: '',
  ref: '',
  statusLabel: '',
  publishLabel: '',
  publishError: '',
  publishErrorOn: false,
  payLabelEditing: false,
  hasLogo: false,
  noLogo: true,
  logo: '',
  title: '',
...
const CHART_VALS_IDLE = {
  line: '',
  area: '',
  labels: [],
  gridVals: [],
  marker: { x: '0', y: '0', value: '' },
  dot: {},
  tip: {},
  src: ''
};
...
const FORM_BLANK = {
  ...
  stmtPeriodTo: '',
  bankDemoPick: '',
  bankDemoLoginId: 'demo_user',
  bankDemoLoginPw: 'demo1234',
  bankDemoOtp: '482913',
  bankDemoAccountsPicked: '',
  reminderDraft: '',
  reminderClient: '',
  reminderTarget: '',
  aiChatDraft: ''
...
const UPCOMING_GATEWAYS = [
  { n: 'Dibsy', initials: 'DY', noteKey: 'ui.pay.gwDibsy' },
  { n: 'QPay', initials: 'QP', noteKey: 'ui.pay.gwQpay' },
  { n: 'Stripe', initials: 'ST', noteKey: 'ui.pay.gwStripe' },
  { n: 'Tap Payments', initials: 'TP', noteKey: 'ui.pay.gwTap' },
  { n: 'Telr', initials: 'TE' },
  { n: 'SADAD', initials: 'SA' },
  { n: 'MyFatoorah', initials: 'MF' },
  { n: 'Fatora', initials: 'FA' },
  { n: 'Noqoody', initials: 'NQ' },
  { n: 'PayTabs', initials: 'PT' }
];
...
export class Component {
  ...
  state = {
    ...
    locale: (() => { try { return readStoredLocale(); } catch (e) { return 'en'; } })(),
    langMenu: false,
    ...
    bankPreview: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).bankPreview || null,
    qatarBanks: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).qatarBanks || [],
    banks: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).banks || [],
    overdueBankCount: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).overdueBankCount || 0,
    statementMonths: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).statementMonths || { empty: true, months: [] },
    env: 'test',
    ppView: 'list',
    ppPages: [],
    ppEditing: null,
    ps: { slug: '', slugCustom: false, theme: 'light', expiry: 'none', after: 'message', closeDate: '', redirectUrl: '' },
    ...
    invFilter: { status: 'All', range: 'All time' },
    invSearch: '',
    invFilterOpen: null,
    form: Object.assign({}, FORM_BLANK),
    toggles: { autoSync: true, recurring: false, twofa: true, biometric: false, testMode: true, productSync: true, combined: false, walletDetect: true, retry: true, monthlySend: false },
    ...
    billingPlans: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).billingPlans || [],
    billingTrialMonths: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).billingTrialMonths || 3,
    ...
    payslips: ((typeof window !== 'undefined' && window.FLOW_DATA) || {}).payslips || [],
    ...
    periodFrom: toDateInput(((typeof window !== 'undefined' && window.FLOW_DATA) || {}).periodFrom, -29),
    periodTo: toDateInput(((typeof window !== 'undefined' && window.FLOW_DATA) || {}).periodTo, 0),
    reminders: [
  ...
  initialsOf(name, fallback) {
    const letters = String(name || '').split(' ').filter(Boolean).map(p => p[0]).join('');
    return (letters || fallback || 'NA').slice(0, 2).toUpperCase();
  }
  digits(text) { return localizeDigits(text, (this.state && this.state.locale) || 'en'); }
  display(text) { return localizeDisplay(text, (this.state && this.state.locale) || 'en'); }
  srcLabel(src) {
    const key = 'overview.src.' + String(src || '').toLowerCase();
    const out = this.t(key);
    return out === key ? src : out;
  }
  tagLabel(tag) {
    const key = 'overview.tag.' + tag;
    const out = this.t(key);
    return out === key ? tag : out;
  }
  tagTree(s) {
    const catalog = (s.tags && s.tags.length) ? s.tags.slice() : ((s.ledgerTags || []).slice());
    const parents = s.tagParents || {};
    const children = {};
    const roots = [];
    catalog.forEach(t => {
      const parent = parents[t];
      if (parent && catalog.indexOf(parent) >= 0 && parent !== t) {
    ...
    return { catalog, parents, children, roots };
  }
  catPicker(s, field) {
    const tree = this.tagTree(s);
    const current = String((s.form && s.form[field]) || '');
    let parent = '';
    let child = '';
    if (tree.roots.indexOf(current) >= 0) {
      parent = current;
    } else if (tree.parents[current] && tree.roots.indexOf(tree.parents[current]) >= 0) {
      parent = tree.parents[current];
      child = current;
    } else if (tree.parents[current] && tree.parents[tree.parents[current]] && tree.roots.indexOf(tree.parents[tree.parents[current]]) >= 0) {
      parent = tree.parents[tree.parents[current]];
      child = tree.parents[current];
    } else if (tree.roots.length) {
      parent = tree.roots[0];
    }
    const kids = tree.children[parent] || [];
    const tag = child || '';
    const grand = tag ? (tree.children[tag] || []) : [];
    let sub = '';
    if (tag && tree.parents[current] === tag) sub = current;
    return {
      value: parent,
      options: tree.roots.map(t => ({ value: t, label: this.tagLabel(t) })),
      set: this.memo('_tc', field, () => (e) => {
        const next = e.target.value;
        this.setState(st => ({ form: Object.assign({}, st.form, { [field]: next }) }));
      }),
      hasTag: kids.length > 0,
      tagValue: tag,
      tagOptions: [{ value: '', label: this.t('ui.exp.noTag') }].concat(kids.map(t => ({ value: t, label: this.tagLabel(t) }))),
      setTag: this.memo('_tgk', field + ':' + parent, () => (e) => {
        const next = e.target.value;
        this.setState(st => ({ form: Object.assign({}, st.form, { [field]: next || parent }) }));
      }),
      hasSub: grand.length > 0,
      subDisabled: grand.length === 0,
      subValue: sub,
      subOptions: [{ value: '', label: this.t('ui.exp.noSub') }].concat(grand.map(t => ({ value: t, label: this.tagLabel(t) }))),
      setSub: this.memo('_ts', field + ':' + tag, () => (e) => {
        const next = e.target.value;
        this.setState(st => ({ form: Object.assign({}, st.form, { [field]: next || tag || parent }) }));
      })
    };
  }
  roleLabel(role) {
    const key = 'pages.role.' + String(role || '').toLowerCase();
    const out = this.t(key);
    return out === key ? role : out;
  }
  statusLabel(status) {
    return translateStatus((key, vars) => this.t(key, vars), status);
  }
  filterLabel(value) {
    const raw = String(value || '');
    const map = {
      All: 'pages.filter.all',
      'All time': 'pages.filter.allTime',
      'Last 7 days': 'pages.filter.last7',
      'Last 30 days': 'pages.filter.last30',
      'This month': 'pages.filter.thisMonth',
      Source: 'pages.filter.source',
      Status: 'pages.filter.status',
      Period: 'pages.filter.period',
      Tag: 'pages.filter.tag',
      Range: 'pages.filter.range'
    };
    if (map[raw]) return this.t(map[raw]);
    const status = this.statusLabel(raw);
    if (status !== raw) return status;
    const src = this.srcLabel(raw);
    if (src !== raw) return src;
    return this.tagLabel(raw);
  }
  periodTitle(label) {
    if (label === 'Today') return this.t('overview.period.today');
    if (label === 'Last 7 days') return this.t('overview.period.last7');
    if (label === 'Last 30 days') return this.t('overview.period.last30');
    return this.display(label || '');
  }
  fmt(n) { const s = n < 0 ? '-' : ''; return this.digits(s + 'QR ' + Math.abs(n).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })); }
  fmt0(n) { const s = n < 0 ? '-' : ''; return this.digits(s + 'QR ' + Math.abs(n).toLocaleString('en-US')); }
  plain(n) { return this.digits(Math.abs(Number(n) || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })); }
  fmtEn(n) { const s = n < 0 ? '-' : ''; return s + 'QR ' + Math.abs(Number(n) || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  plainEn(n) { return Math.abs(Number(n) || 0).toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }); }
  ...
  closeFly(alsoRail) {
    ...
    if (!this.state.fly && !(alsoRail && this.state.railOpen)) return;
    this.setState(alsoRail ? { fly: null, railOpen: false } : { fly: null });
  }
  go(page, tab) { return this.memo('_nav', page + '|' + (tab || ''), () => () => { clearTimeout(this._flyT); this.setState(s => ({ page, detail: null, openTag: null, fly: null, moreOpen: false, tab: tab ? Object.assign({}, s.tab, { [page]: tab }) : s.tab })); }); }
  tabH(page, tab) { return this.memo('_tab', page + '::' + tab, () => () => this.setState(s => ({ tab: Object.assign({}, s.tab, { [page]: tab }), detail: null, openTag: page === 'settings' && tab === 'tags' ? s.openTag : null }))); }
  open(type, id) { return this.memo('_open', type + ':' + id, () => () => this.setState({ detail: { type, id } })); }
  toast(msg, ms) { clearTimeout(this._tt); this.setState({ toast: msg }); this._tt = setTimeout(() => this.setState({ toast: '' }), ms || 2600); }
  toggle(k) { return this.memo('_tg', k, () => () => this.setState(s => ({ toggles: Object.assign({}, s.toggles, { [k]: !s.toggles[k] }) }))); }
  openModal(kind) {
    return this.memo('_md', kind, () => () => this.setState(s => this.modalOpenState(s, kind)));
  }
  openStatementForAccount(accountId) {
    this.setState(s => this.modalOpenState(s, 'statement', { stmtAccount: accountId }));
  }
  modalOpenState(s, kind, extras) {
    const KEYS = {
      ...
      client: ['cliName', 'cliEmail', 'cliPhone', 'cliAddress'],
      ...
      tag: ['newTag', 'newTagParent'],
      statement: ['stmtAccount', 'stmtBankName', 'stmtLabel', 'stmtCsv', 'stmtFile', 'stmtPeriodFrom', 'stmtPeriodTo'],
      bankDemo: ['bankDemoPick', 'bankDemoLoginId', 'bankDemoLoginPw', 'bankDemoOtp', 'bankDemoAccountsPicked'],
      reminderPreview: ['reminderDraft', 'reminderClient', 'reminderTarget']
    };
    const form = Object.assign({}, s.form);
    (KEYS[kind] || []).forEach(k => {
      form[k] = Object.prototype.hasOwnProperty.call(FORM_BLANK, k) ? FORM_BLANK[k] : '';
    });
    ...
    if (kind === 'statement') {
      const forced = extras && extras.stmtAccount;
      form.stmtAccount = forced ? forced : this.defaultStmtAccount(s.banks);
      const catalog = this.qatarBankCatalog();
      form.stmtBankName = (catalog[0] && catalog[0].bank) || '';
    }
    if (kind === 'bankDemo') {
      const catalog = this.qatarBankCatalog();
      form.bankDemoPick = (catalog[0] && catalog[0].bank) || '';
      form.bankDemoLoginId = 'demo_user';
      form.bankDemoLoginPw = 'demo1234';
      form.bankDemoOtp = '482913';
      form.bankDemoAccountsPicked = ['current', 'savings'];
    }
    return {
      modal: kind, scanDone: false, scanBusy: false, scanLines: [], scanConf: {},
      editEmp: null, editClient: null, editTag: null, form, invDueError: '',
      bankDemoStep: kind === 'bankDemo' ? 0 : s.bankDemoStep
    };
  }
  defaultStmtAccount(banks) {
    let pick = null;
    (banks || []).forEach(row => {
      if (!row || row.sample) return;
      if (row.lastImportOffset == null || !Number.isFinite(Number(row.lastImportOffset))) return;
      if (!pick || Number(row.lastImportOffset) > Number(pick.lastImportOffset)) pick = row;
    });
    return (pick && pick.id) || '__new__';
  }
  ...
  filtered() {
    const { search, filter, txns } = this.state;
    const q = search.trim().toLowerCase();
    const cache = this._filteredTxnsCache;
    if (
      cache &&
      cache.txns === txns &&
      cache.q === q &&
      cache.source === filter.source &&
      cache.status === filter.status &&
      cache.tag === filter.tag &&
      cache.range === filter.range
    ) {
      return cache.output;
    }
    const output = this.sortedTxns().filter(t =>
    ...
    this._filteredTxnsCache = {
      txns, q,
      source: filter.source, status: filter.status, tag: filter.tag, range: filter.range,
      output
    };
    return output;
  }
  ...
  applyStore(extra) {
    ...
    const snap = {
      ...
      attention: live.attention, scan: live.scan,       sampleBills: live.sampleBills, clients: live.clients, team: live.team, employees: live.employees, payslips: live.payslips,
      ...
      sampleBanks: live.sampleBanks, bankPreview: live.bankPreview, qatarBanks: live.qatarBanks, banks: live.banks, overdueBankCount: live.overdueBankCount, statementMonths: live.statementMonths, merchantName: live.merchantName,
      ownerName: live.ownerName, accountantName: live.accountantName, acctName: live.acctName,
      acctEmail: live.acctEmail, profile: live.profile, plan: live.plan, billingPlans: live.billingPlans, billingTrialMonths: live.billingTrialMonths, showTax: live.showTax,
      ...
      ledgerTags: live.ledgerTags, tags: live.tags, tagParents: live.tagParents
    };
    if (!this._ratesInited && live.figures && live.figures.deductionRate != null) {
      snap.rates = Object.assign({}, this.state.rates, { wht: String(Math.round(live.figures.deductionRate * 100)) });
      this._ratesInited = true;
    }
  ...
  persistCreatedInvoice(opts) {
    ...
    const issuedRaw = opts && opts.issuedRaw;
    let issuedOffset = 0;
    if (issuedRaw != null && String(issuedRaw).trim() !== '') {
      const parsedIssued = this.dueOffsetFrom(issuedRaw);
      if (parsedIssued == null) {
        return { error: 'Issue date is invalid', field: 'issued' };
      }
      issuedOffset = parsedIssued;
    }
    if (issuedOffset > dueOffset) {
      return { error: 'Issue date cannot be after the due date', field: 'issued' };
    }
    ...
    try {
      const invoice = FlowStore.createInvoice({
        ...
        draft: !!opts.draft,
        lines: opts.lines || [],
        partialPayment: !!opts.partialPayment,
        discountMinor: Math.round(opts.discountMinor || 0),
  ...
  sendInvoiceReminder(name) {
    this.toast(name ? this.t('chrome.toast.reminderSent', { name }) : this.t('chrome.toast.reminderSentClient'));
  }

  draftReminderMessage(row) {
    const client = row.client || this.t('ui.rem.draftClient');
    const amt = row.amt || this.fmt(row.amount || 0);
    const due = row.due || '';
    const overdue = row.status === 'Overdue' || (row.daysLate && row.daysLate > 0);
    if (overdue) {
      return this.t('ui.rem.draftOverdue', { client, amt, due });
    }
    return this.t('ui.rem.draftUpcoming', { client, amt, due });
  }

  openReminderPreview(row) {
    this.setState(s => {
      const next = this.modalOpenState(s, 'reminderPreview');
      next.form = Object.assign({}, next.form, {
        reminderDraft: this.draftReminderMessage(row),
        reminderClient: row.client || '',
        reminderTarget: row.id || ''
      });
      return next;
    });
  }
  ...
  answerAiChat(query) {
    ...
    const period = this.reportsPeriod();
    const runway = period.runway || {};
    const ageing = s.ageing || { amountMinor: 0, count: 0, rows: [] };
    const cash = runway.cashText || this.fmt0(runway.cash || 0);

    if (/cash|runway|burn|نقد|مدرج|حرق/.test(q)) {
      if (runway.profitable) return this.t('ui.ai.chatCashPositive', { cash });
      return this.t('ui.ai.chatCash', { cash, months: runway.months || this.t('ui.ai.chatMonthsUnknown') });
    }
    if (/overdue|receivable|unpaid|owe|متأخر|مستحق|يدين/.test(q)) {
      if (!ageing.count) return this.t('ui.ai.chatOverdueNone');
      const top = (ageing.rows || []).slice().sort((a, b) => (b.amount || 0) - (a.amount || 0))[0];
      const topBit = top
        ? this.t('ui.ai.chatOverdueTop', { client: top.client || this.t('ui.rem.draftClient'), amt: top.amt || this.fmt0(top.amount || 0) })
        : '';
      return this.t(ageing.count === 1 ? 'ui.ai.chatOverdueOne' : 'ui.ai.chatOverdueMany', {
        count: ageing.count,
        amt: this.fmt0((s.invoiceTotals && s.invoiceTotals.outstanding) || (ageing.amountMinor || 0) / 100),
        top: topBit
      });
    }
    if (/spend|expense|cost|إنفاق|مصروف|أنفق/.test(q)) {
      const spend = period.spend || {};
      return spend.insight || this.t('ui.ai.chatSpendEmpty');
    }
    if (/revenue|income|sales|إيراد|دخل|مبيعات/.test(q)) {
      return period.netTrendText ? this.t('ui.ai.chatRevenue', { trend: period.netTrendText }) : this.t('ui.ai.chatRevenueEmpty');
    }
    return this.t('ui.ai.chatFallback');
  }

  sendAiChatMessage(presetQuery) {
    const draft = presetQuery != null ? presetQuery : ((this.state.form && this.state.form.aiChatDraft) || '');
    if (!draft.trim()) return;
    this.setState(s => ({
      aiChatMessages: (s.aiChatMessages || []).concat([{ role: 'user', text: draft }]),
      aiChatThinking: true,
      form: Object.assign({}, s.form, { aiChatDraft: '' })
    }));
    clearTimeout(this._aiChatT);
    this._aiChatT = setTimeout(() => {
      const answer = this.display(this.answerAiChat(draft));
      this.setState(s => ({
        aiChatMessages: (s.aiChatMessages || []).concat([{ role: 'ai', text: answer }]),
        aiChatThinking: false
      }));
    }, 500 + Math.random() * 350);
  }
  ...
  postPayrollToLedger() {
    ...
    if (FlowStore.payrollPostedFor && FlowStore.payrollPostedFor(period)) {
      this.toast(this.t('ui.payr.alreadyPosted', { period: this.display(period) }));
      return;
    }
    try {
      const result = FlowStore.postPayroll(period);
      if (result.alreadyPosted) {
        this.toast(this.t('ui.payr.alreadyPosted', { period: this.display(result.period || period) }));
        return;
      }
      this.applyStore();
  ...
  generatePayslipsForPeriod() {
    const period = (this.state.form && this.state.form.payrollPeriod)
      || (typeof FlowStore !== 'undefined' && FlowStore.defaultPayrollPeriod && FlowStore.defaultPayrollPeriod())
      || '';
    if (typeof FlowStore === 'undefined' || !FlowStore.generatePayslips) {
      this.toast(this.t('ui.payr.genFailed'));
      return;
    }
    try {
      const slips = FlowStore.generatePayslips(period);
      this.applyStore();
      const count = (slips && slips.length) || 0;
      this.toast(this.t('ui.payr.generated', { count }));
    } catch (err) {
      this.toast((err && err.message) || this.t('ui.payr.genFailed'));
    }
  }

  downloadCurrentPayslipPdf(period, name) {
    if (this._pdfBusy) return;
    const node = typeof document !== 'undefined' ? document.querySelector('.flow-payslip-doc') : null;
    this._pdfBusy = true;
    this.setState({ invoicePdfBusy: true });
    downloadInvoicePdf(period, name, node).then(filename => {
      this.toast('Downloaded ' + filename);
    }).catch(err => {
      console.error('[pdf] download failed', err);
      this.toast((err && err.message) || 'Could not download PDF');
    }).finally(() => {
      this._pdfBusy = false;
      this.setState({ invoicePdfBusy: false });
    });
  }
  ...
  downloadCurrentInvoicePdf(number, client) {
    if (this._pdfBusy) return;
    const node = typeof document !== 'undefined' ? document.querySelector('.flow-invoice-doc') : null;
    this._pdfBusy = true;
    this.setState({ invoicePdfBusy: true });
    downloadInvoicePdf(number, client, node).then(filename => {
      this.toast('Downloaded ' + filename);
    }).catch(err => {
      console.error('[pdf] download failed', err);
      this.toast((err && err.message) || 'Could not download PDF');
  ...
  printCurrentInvoice() {
    try {
      ...
      this.toast((err && err.message) || 'Could not print invoice');
    }
  }
  ...
  row(t) {
    return Object.assign({}, t, {
      ...
      d: this.display(t.d), src: this.srcLabel(t.src), tag: this.tagLabel(t.tag),
  ...
  filterOpts(key, list) {
    ...
    return list.map(v => {
      const on = cur === v;
      return {
        label: this.filterLabel(v), on, off: !on, go: this.setFilter(key, v),
  ...
  inInvRange(inv, range) {
    if (range === 'All time' || !range) return true;
    const offset = Number(inv.issuedOffset);
    if (Number.isFinite(offset)) {
      if (range === 'Last 7 days') return offset >= -6 && offset <= 0;
      if (range === 'Last 30 days') return offset >= -29 && offset <= 0;
      if (range === 'This month') return !!inv.issuedThisMonth;
    }
    return this.inRange(inv.issued, range);
  }

  filteredInvoices(list) {
    const invFilter = this.state.invFilter || { status: 'All', range: 'All time' };
    const q = String(this.state.invSearch || '').trim().toLowerCase();
    return (list || []).filter(i => {
      const hay = (String(i.no || '') + ' ' + String(i.client || '')).toLowerCase();
      return (!q || hay.includes(q))
        && (invFilter.status === 'All' || i.status === invFilter.status)
        && this.inInvRange(i, invFilter.range);
    });
  }

  invFilterMenu(key, list, label) {
    const cur = (this.state.invFilter || {})[key] || (key === 'range' ? 'All time' : 'All');
    const active = cur !== 'All' && cur !== 'All time';
    const open = this.state.invFilterOpen === key;
    return {
      label: this.filterLabel(label), value: this.filterLabel(cur), active, open,
      ...
      toggle: this.memo('_ifm', key, () => () => this.setState(s => ({ invFilterOpen: s.invFilterOpen === key ? null : key }))),
      options: list.map(v => ({
        label: this.filterLabel(v), on: v === cur,
        go: this.memo('_ifmset', key + v, () => () => this.setState(s => ({ invFilter: Object.assign({}, s.invFilter || { status: 'All', range: 'All time' }, { [key]: v }), invFilterOpen: null })))
      }))
    };
  }
  ...
  retag(id, tag) { return this.memo('_rt', id + tag, () => () => { this.setState(s => ({ txns: s.txns.map(t => t.id === id ? Object.assign({}, t, { tag }) : t) })); this.toast(this.t('chrome.toast.tagUpdated')); }); }
  ...
  submitModal = () => {
    ...
    if (k === 'link') {
      ...
      const finish = (payUrl, copied) => {
        this.applyStore({ modal: null });
        this.toast(copied ? this.t('chrome.toast.linkCreatedCopied') : this.t('chrome.toast.linkCreated'));
      };
      ...
      this.toast(this.t('chrome.toast.linkCreated'));
    } else if (k === 'expense') {
      appendOut({
      ...
      this.setState(st => ({
        page: 'transactions',
        detail: null,
        tab: Object.assign({}, st.tab, { transactions: 'all' })
      }));
      this.toast('Bill saved to Transactions', 4000);
    } else if (k === 'invoice') {
      ...
      const name = String(f.cliName || '').trim();
      if (!name) { this.toast('Client name is required'); return; }
      const payload = {
        name,
        email: String(f.cliEmail || '').trim(),
        phone: String(f.cliPhone || '').trim(),
        address: String(f.cliAddress || '').trim()
      };
      if (s.editClient) {
        if (typeof FlowStore === 'undefined' || !FlowStore.updateClient) {
          this.toast('Could not update client');
          return;
        }
        try {
          FlowStore.updateClient(s.editClient, payload);
          this.applyStore({ modal: null, editClient: null });
          this.toast('Client updated');
        } catch (err) {
          this.toast((err && err.message) || 'Could not update client');
        }
        return;
      }
      if (typeof FlowStore === 'undefined' || !FlowStore.addClient) {
        this.toast('Could not add client');
        return;
      }
      try {
        FlowStore.addClient(payload);
        this.applyStore({ modal: null, editClient: null });
        this.toast(name + ' added');
      } catch (err) {
        this.toast((err && err.message) || 'Could not add client');
      }
    } else if (k === 'member') {
      ...
      const name = String(f.newTag || '').trim();
      const parent = String(f.newTagParent || '').trim();
      if (!name) { this.toast(this.t('chrome.toast.tagRequired')); return; }
      if (s.editTag) {
        const from = s.editTag;
        const liveParents = s.tagParents || {};
        const currentParent = liveParents[from] || '';
        if (typeof FlowStore === 'undefined' || !FlowStore.renameTag) { this.toast(this.t('chrome.toast.tagRenameFailed')); return; }
        if (name === from && parent === currentParent) { this.toast(this.t('chrome.toast.tagUnchanged')); return; }
        try {
          const result = FlowStore.renameTag(from, name, parent);
          this.applyStore({ modal: null, editTag: null, openTag: s.openTag === from ? name : s.openTag });
          if (name !== from) {
            this.toast(result.count ? this.t('chrome.toast.tagRenamedTo', { from, to: name }) : this.t('chrome.toast.tagRenamed'));
          } else if (parent) {
            this.toast(this.t('chrome.toast.tagParentSet', { name, parent }));
          } else {
            this.toast(this.t('chrome.toast.tagParentCleared', { name }));
          }
        } catch (err) {
          this.toast((err && err.message) || this.t('chrome.toast.tagParentFailed'));
        }
      } else {
        if (typeof FlowStore === 'undefined' || !FlowStore.createTag) { this.toast(this.t('chrome.toast.tagCreateFailed')); return; }
        try {
          FlowStore.createTag(name, parent || null);
          this.applyStore({ modal: null, editTag: null });
          this.toast(parent ? this.t('chrome.toast.tagCreatedUnder', { name, parent }) : this.t('chrome.toast.tagCreated', { name }));
        } catch (err) {
          this.toast((err && err.message) || this.t('chrome.toast.tagCreateFailed'));
        }
      }
    } else if (k === 'statement') {
      const csv = String(f.stmtCsv || '');
      if (!csv.trim()) { this.toast(this.t('chrome.toast.stmtNoFile')); return; }
      if (typeof FlowStore === 'undefined' || !FlowStore.importBankStatement) {
        this.toast(this.t('chrome.toast.stmtFailed'));
        return;
      }
      const accountId = String(f.stmtAccount || '');
      const creating = !accountId || accountId === '__new__';
      try {
        const result = FlowStore.importBankStatement({
          csvText: csv,
          accountId: creating ? null : accountId,
          bankName: creating ? f.stmtBankName : undefined,
          label: creating ? f.stmtLabel : undefined,
          periodFromRaw: f.stmtPeriodFrom,
          periodToRaw: f.stmtPeriodTo
        });
        this.applyStore({ modal: null });
        const badDate = result.skippedReasons && result.skippedReasons.badDate;
        if (badDate > 0) {
          this.toast(this.t('chrome.toast.stmtBadDate', { count: result.imported, skipped: badDate }), 4000);
        } else {
          this.toast(this.t('chrome.toast.stmtImported', { count: result.imported, skipped: result.skipped }), 4000);
        }
      } catch (err) {
        this.toast((err && err.message) || this.t('chrome.toast.stmtFailed'));
      }
    } else if (k === 'bankDemo') {
      if ((s.bankDemoStep || 0) < 4) {
        this.setState(st => ({ bankDemoStep: (st.bankDemoStep || 0) + 1 }));
        return;
      }
      this.setState({ modal: null });
    } else if (k === 'reset') {
      this.runResetDemo();
    } else if (k === 'removeBank') {
      this.confirmRemoveBank();
    } else if (k === 'reminderPreview') {
      this.sendInvoiceReminder(s.form.reminderClient);
      this.setState({ modal: null });
      return;
    } else if (k === 'pageSettings') {
  ...
  checkoutSettingsFromState(st) {
    const ps = (st && st.ps) || {};
    const rc = (st && st.rc) || {};
    return {
      slug: this.pageSlug(st),
      theme: ps.theme === 'dark' ? 'dark' : 'light',
      closeMode: ps.expiry === 'date' ? 'date' : 'none',
      closeLabel: ps.expiry === 'date' ? String(ps.closeDate || '') : '',
      afterPay: ps.after === 'redirect' ? 'redirect' : 'message',
      redirectUrl: ps.after === 'redirect' ? String(ps.redirectUrl || '').trim() : '',
      receiptAuto: rc.auto !== false,
      receiptCustomer: !!rc.showCustomer,
      receiptRef: !!rc.ref
    };
  }
  ...
  savePageSettings() {
    const st = this.state;
    const patch = this.checkoutSettingsFromState(st);
    if (st.ppEditing && typeof FlowStore !== 'undefined' && FlowStore.saveCheckoutSettings) {
      try {
        const page = FlowStore.saveCheckoutSettings(st.ppEditing, {
          slug: patch.slug,
          theme: patch.theme,
          closeMode: patch.closeMode,
          closeLabel: patch.closeLabel,
          afterPay: patch.afterPay,
          redirectUrl: patch.redirectUrl
        });
        this.applyStore({
          modal: null,
        ...
        this.toast((err && err.message) || 'Could not save page settings');
        return;
      }
    } else {
      this.setState({ modal: null });
    }
    this.toast('Page settings saved');
  }
  ...
  saveReceiptSettings() {
    ...
    if (st.ppEditing && typeof FlowStore !== 'undefined' && FlowStore.saveCheckoutSettings) {
      try {
        FlowStore.saveCheckoutSettings(st.ppEditing, {
          receiptAuto: patch.receiptAuto,
          receiptCustomer: patch.receiptCustomer,
          receiptRef: patch.receiptRef
        });
        this.applyStore({ modal: null });
      } catch (err) {
        this.toast((err && err.message) || 'Could not save receipt settings');
        return;
      }
    } else {
      this.setState({ modal: null });
    }
    this.toast('Receipt settings saved');
  }
  ...
  runResetDemo() {
    ...
    this.toast(this.t('chrome.toast.demoRestored'));
  }

  askRemoveBank(id) {
    const row = (this.state.banks || []).find(b => b.id === id);
    if (!row || !row.sample) {
      this.toast(this.t('chrome.toast.sampleRemoveDenied'));
      return;
    }
    this.setState({ modal: 'removeBank', removeBankId: id });
  }

  confirmRemoveBank() {
    const id = this.state.removeBankId;
    const row = (this.state.banks || []).find(b => b.id === id);
    if (!row || !row.sample) {
      this.toast(this.t('chrome.toast.sampleRemoveDenied'));
      this.setState({ modal: null, removeBankId: null });
      return;
    }
    if (typeof FlowStore === 'undefined' || !FlowStore.removeBankAccount) {
      this.toast(this.t('chrome.toast.sampleRemoveDenied'));
      this.setState({ modal: null, removeBankId: null });
      return;
    }
    const removed = FlowStore.removeBankAccount(id);
    if (!removed) {
      this.toast(this.t('chrome.toast.sampleRemoveDenied'));
      this.setState({ modal: null, removeBankId: null });
      return;
    }
    this.applyStore({ modal: null, removeBankId: null });
    this.toast(this.t('chrome.toast.sampleRemoved'));
  }

  copyPayUrl(url, quiet) {
    let text = url || '';
    ...
    if (clip && text) {
      Promise.resolve(navigator.clipboard.writeText(text)).then(() => {
        if (!quiet) this.toast(this.t('chrome.toast.linkCopied'));
      }).catch(() => {
        if (!quiet) this.toast(this.t('chrome.toast.linkCopied'));
      });
      return;
    }
    if (!quiet) this.toast(text ? this.t('chrome.toast.linkCopied') : this.t('chrome.toast.noLink'));
  }

  invoiceShareUrl(number) {
  ...
  publishCheckout() {
    ...
    const amountMode = st.ppAmountMode === 'open' || st.ppAmountMode === 'qty' ? st.ppAmountMode : 'fixed';
    const major = Math.abs(parseFloat(String((priceField && priceField.unitPrice) || '').replace(/[^0-9.]/g, '')) || 0);
    const amountMinor = amountMode === 'open' ? 0 : Math.round(major * 100);
    if (!String(st.pp.title || '').trim()) {
      this.setState({ ppPublishError: 'Page title is required' });
      return;
    }
    if (amountMode !== 'open' && !amountMinor) {
    ...
    try {
      const page = FlowStore.publishCheckoutPage({
        ...
        ...this.checkoutSettingsFromState(st)
      });
      const origin = typeof location !== 'undefined' ? location.origin : '';
      this.applyStore({
  ...
  applyReview(id, action, invNo, stay) {
    if (action !== 'reject' && typeof FlowStore !== 'undefined' && FlowStore.confirmMatch) {
      ...
      this.toast(invNo ? this.t('chrome.toast.linkedTo', { inv: invNo }) : this.t('chrome.toast.matchConfirmed'));
      return;
    }
    this.setState(s => {
    ...
    this.toast(action === 'reject' ? this.t('chrome.toast.sentBack') : (invNo ? this.t('chrome.toast.linkedTo', { inv: invNo }) : this.t('chrome.toast.matchConfirmed')));
  }
  ...
  reportsPeriod() {
    ...
    return (this.state.periods && this.state.periods[tf]) || { moneyIn: 0, moneyOut: 0, pending: 0, net: 0, label: 'Last 30 days', series: { labels: [], values: [] }, pnl: {}, branches: [], runway: {}, spend: { tags: [], vendors: [] }, forecast: [], forecastSummary: { hasProjection: false, netMinor: 0, netText: '', positive: true }, range: '' };
  }
  series() {
  ...
  chart() {
    ...
    const svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 720 190" preserveAspectRatio="none">' +
      ...
      '</svg>';
    return {
      line: d, area,       labels: l.map(t => ({ t: this.display(t), axisStyle: this.state.tf === 'month' ? 'flex:1; min-width:0; white-space:nowrap' : '' })),
      gridVals: gridVals.map(g => Object.assign({}, g, { label: this.display(g.label) })),
      marker: { x: p[0].toFixed(1), y: p[1].toFixed(1), value: this.display(valueText) },
      ...
      tip: {
        ...
        value: this.display(valueText)
      },
      src: 'data:image/svg+xml,' + encodeURIComponent(svg)
    };
  }
  attachChrome() { this.componentDidMount(); }
  detachChrome() { this.componentWillUnmount(); }
  t(key, vars) {
    const locale = (this.state && this.state.locale) || 'en';
    if (!this._t || this._tLocale !== locale) {
      this._tLocale = locale;
      this._t = createTranslator(locale);
    }
    return this._t(key, vars);
  }
  componentDidMount() {
    applyLocaleDir((this.state && this.state.locale) || 'en');
    this._onResize = () => {
    ...
    this._onKey = (e) => {
      if ((e.metaKey || e.ctrlKey) && (e.key === 'k' || e.key === 'K')) {
        ...
        this.setState(s => (s.modal ? { modal: null, editEmp: null, editClient: null, editTag: null } : s.invFilterOpen ? { invFilterOpen: null } : s.searchOpen ? { searchOpen: false, search: '' } : s.aiChatOpen ? { aiChatOpen: false } : s.moreOpen ? { moreOpen: false } : s.langMenu ? { langMenu: false } : s.userMenu ? { userMenu: false } : s.fly ? { fly: null, railOpen: false } : s.pickFor ? { pickFor: null } : s.detail ? { detail: null } : null));
      }
    };
    this._onPointer = (e) => {
      const t = e.target;
      if (this.state.fly && !(t && t.closest && (t.closest('.flow-open-menu') || t.closest('aside')))) {
        this.closeFly(true);
      }
      const patch = {};
      if (this.state.userMenu && !(t && t.closest && t.closest('[data-user-menu]'))) patch.userMenu = false;
      if (this.state.langMenu && !(t && t.closest && t.closest('[data-lang-menu]'))) patch.langMenu = false;
      if (Object.keys(patch).length) this.setState(patch);
    };
    this._onWheel = () => {
      if (this.state.fly) this.closeFly(true);
    };
    window.addEventListener('keydown', this._onKey);
    document.addEventListener('pointerdown', this._onPointer, true);
    document.addEventListener('wheel', this._onWheel, { capture: true, passive: true });
  ...
  headFor() {
    ...
    if (d) {
      const key = 'pages.detail.' + (d.type || 'fallback');
      const title = this.t(key + '.title');
      return { title: title.indexOf('pages.detail.') === 0 ? this.t('pages.detail.fallback.title') : title, sub: this.t(key + '.sub') };
    }
    if (this.state.page === 'settings') return { title: this.t('settings.head.title'), sub: this.t('settings.head.sub') };
    return { title: this.t('pages.head.' + this.state.page + '.title'), sub: this.t('pages.head.' + this.state.page + '.sub') };
  }
  ...
  undoMatch(id) {
    ...
    this.toast(this.t('chrome.toast.matchUndone'));
  }
  ...
  backLabel() {
    ...
    const [page, tab] = prev.split('|');
    const { D } = this.dests();
    const SEC = { dashboard: this.t('chrome.nav.dashboard'), payments: this.t('chrome.nav.payments'), transactions: this.t('chrome.nav.transactions'), invoicing: this.t('chrome.nav.invoicing'), accounting: this.t('chrome.nav.accounting'), connections: this.t('chrome.nav.connections'), reports: this.t('chrome.nav.reports'), team: this.t('chrome.nav.team'), payroll: this.t('chrome.nav.payroll'), settings: this.t('chrome.nav.settings') };
    if (page === 'dashboard') return this.t('chrome.nav.dashboard');
    if (tab === 'hub' || !tab) return (SEC[page] || this.t('chrome.nav.dashboard')) + ' · ' + this.t('chrome.mobile.overview');
    const e = (D[page] || []).find(x => x[0] === tab);
    return e ? e[1] : (SEC[page] || this.t('chrome.nav.dashboard'));
  }
  dflt(p) {
  ...
  needPage(page, tab) {
    ...
    if (tab != null && this.tabOf(page, this.dflt(page)) !== tab) return false;
    return true;
  }
  dests() {
    const D = {
      payments: [
        ['links', this.t('pages.dest.payments.links.title'), this.t('pages.dest.payments.links.sub'), 'link'],
        ['checkout', this.t('pages.dest.payments.checkout.title'), this.t('pages.dest.payments.checkout.sub'), 'page'],
        ['subs', this.t('pages.dest.payments.subs.title'), this.t('pages.dest.payments.subs.sub'), 'repeat'],
        ['gateway', this.t('pages.dest.payments.gateway.title'), this.t('pages.dest.payments.gateway.sub'), 'gear'],
        ['bank', this.t('pages.dest.payments.bank.title'), this.t('pages.dest.payments.bank.sub'), 'bank'],
        ['shopify', this.t('pages.dest.payments.shopify.title'), this.t('pages.dest.payments.shopify.sub'), 'store']
      ],
      transactions: [
        ['all', this.t('pages.dest.transactions.all.title'), this.t('pages.dest.transactions.all.sub'), 'list'],
        ['matching', this.t('pages.dest.transactions.matching.title'), this.t('pages.dest.transactions.matching.sub'), 'match'],
        ['scan', this.t('pages.dest.transactions.scan.title'), this.t('pages.dest.transactions.scan.sub'), 'scan'],
        ['bank', this.t('pages.dest.transactions.bank.title'), this.t('pages.dest.transactions.bank.sub'), 'bank']
      ],
      invoicing: [
        ['all', this.t('pages.dest.invoicing.all.title'), this.t('pages.dest.invoicing.all.sub'), 'list'],
        ['create', this.t('pages.dest.invoicing.create.title'), this.t('pages.dest.invoicing.create.sub'), 'doc'],
        ['reminders', this.t('pages.dest.invoicing.reminders.title'), this.t('pages.dest.invoicing.reminders.sub'), 'bell'],
        ['recurring', this.t('pages.dest.invoicing.recurring.title'), this.t('pages.dest.invoicing.recurring.sub'), 'repeat'],
        ['clients', this.t('pages.dest.invoicing.clients.title'), this.t('pages.dest.invoicing.clients.sub'), 'people']
      ],
      accounting: [
        ['tally', this.t('pages.dest.accounting.tally.title'), this.t('pages.dest.accounting.tally.sub'), 'doc'],
        ['zoho', this.t('pages.dest.accounting.zoho.title'), this.t('pages.dest.accounting.zoho.sub'), 'repeat'],
        ['tax', this.t('pages.dest.accounting.tax.title'), this.t('pages.dest.accounting.tax.sub'), 'cash']
      ],
      connections: [
        ['gateways', this.t('pages.dest.connections.gateways.title'), this.t('pages.dest.connections.gateways.sub'), 'card'],
        ['banks', this.t('pages.dest.connections.banks.title'), this.t('pages.dest.connections.banks.sub'), 'bank'],
        ['platforms', this.t('pages.dest.connections.platforms.title'), this.t('pages.dest.connections.platforms.sub'), 'repeat']
      ],
      reports: [
        ['overview', this.t('pages.dest.reports.overview.title'), this.t('pages.dest.reports.overview.sub'), 'chart'],
        ['spend', this.t('pages.dest.reports.spend.title'), this.t('pages.dest.reports.spend.sub'), 'tag'],
        ['cash', this.t('pages.dest.reports.cash.title'), this.t('pages.dest.reports.cash.sub'), 'cash'],
        ['receivables', this.t('pages.dest.reports.receivables.title'), this.t('pages.dest.reports.receivables.sub'), 'clock'],
        ['branches', this.t('pages.dest.reports.branches.title'), this.t('pages.dest.reports.branches.sub'), 'building'],
        ['pack', this.t('pages.dest.reports.pack.title'), this.t('pages.dest.reports.pack.sub'), 'doc']
      ],
      team: [
        ['members', this.t('pages.dest.team.members.title'), this.t('pages.dest.team.members.sub'), 'people'],
        ['approvals', this.t('pages.dest.team.approvals.title'), this.t('pages.dest.team.approvals.sub'), 'match'],
        ['permissions', this.t('pages.dest.team.permissions.title'), this.t('pages.dest.team.permissions.sub'), 'shield'],
        ['audit', this.t('pages.dest.team.audit.title'), this.t('pages.dest.team.audit.sub'), 'clock']
      ],
      payroll: [
        ['payslips', this.t('pages.dest.payroll.payslips.title'), this.t('pages.dest.payroll.payslips.sub'), 'doc'],
        ['employees', this.t('pages.dest.payroll.employees.title'), this.t('pages.dest.payroll.employees.sub'), 'people'],
        ['tax', this.t('pages.dest.payroll.tax.title'), this.t('pages.dest.payroll.tax.sub'), 'cash']
      ],
      settings: [
        ['profile', this.t('settings.tabs.profile'), this.t('settings.tabs.profileSub'), 'building'],
        ['account', this.t('settings.tabs.account'), this.t('settings.tabs.accountSub'), 'gear'],
        ['tags', this.t('settings.tabs.tags'), this.t('settings.tabs.tagsSub'), 'tag'],
        ['billing', this.t('settings.tabs.billing'), this.t('settings.tabs.billingSub'), 'cash'],
        ['security', this.t('settings.tabs.security'), this.t('settings.tabs.securitySub'), 'shield']
      ]
    };
  ...
  qatarBankCatalog() {
    const fromState = this.state && this.state.qatarBanks;
    if (Array.isArray(fromState) && fromState.length) return fromState;
    if (typeof FlowStore !== 'undefined' && Array.isArray(FlowStore.QATAR_BANKS) && FlowStore.QATAR_BANKS.length) {
      return FlowStore.QATAR_BANKS;
    }
    return [];
  }

  qatarBankPickerRows(selectedBank, memoPrefix, field) {
    return this.qatarBankCatalog().map(b => {
      const bank = b.bank;
      const logo = b.logo != null
        ? b.logo
        : (typeof FlowStore !== 'undefined' && FlowStore.bankLogoSrc ? FlowStore.bankLogoSrc(bank) : null);
      return {
        id: b.id,
        bank,
        label: b.short ? (b.bank + ' (' + b.short + ')') : b.bank,
        logo: logo || null,
        initials: b.initials || this.initialsOf(bank),
        on: bank === selectedBank,
        go: this.memo(memoPrefix, b.id, () => () => this.setState(st => ({
          form: Object.assign({}, st.form, { [field]: bank })
        })))
      };
    });
  }
  ...
  chip(status) {
    const map = {
      ...
      'Coming soon': ['var(--ink-3)', 'var(--chip)'],
      'Up to date': ['var(--pos)', 'var(--pos-soft)'],
      'Statement due soon': ['var(--warn)', 'var(--warn-soft)'],
      'Never uploaded': ['var(--ink-3)', 'var(--chip)'],
  ...
  reportsPageVals(s) {
    ...
    const pnl = {
      ...
      cards: [
        { label: this.t('ui.rep.lblRevenue'), val: pnlText.revenue ? this.digits(pnlText.revenue) : pnlText.revenue, color: 'var(--ink)', note: this.periodTitle(periodLabel) },
        { label: this.t('ui.rep.lblCogs'), val: pnlText.costOfSalesCard ? this.digits(pnlText.costOfSalesCard) : pnlText.costOfSalesCard, color: 'var(--ink)', note: this.t('ui.rep.noteCogs') },
        { label: this.t('ui.rep.lblOver'), val: pnlText.overheadsCard ? this.digits(pnlText.overheadsCard) : pnlText.overheadsCard, color: 'var(--ink)', note: this.t('ui.rep.noteOver') },
        { label: this.t('ui.rep.lblNet'), val: pnlText.netProfit ? this.digits(pnlText.netProfit) : pnlText.netProfit, color: netProfit >= 0 ? 'var(--pos)' : 'var(--neg)', note: this.t('ui.rep.noteMargin', { pct: pnlSrc.margin != null ? pnlSrc.margin : 0 }) }
      ],
      rows: [
        { label: this.t('ui.rep.revenue'), val: pnlText.revenue ? this.digits(pnlText.revenue) : pnlText.revenue, bar: barW(inn), color: 'var(--pos)', weight: '650' },
        { label: this.t('ui.rep.refunds'), val: pnlText.refunds ? this.digits(pnlText.refunds) : pnlText.refunds, bar: barW(refunds), color: 'var(--ink-2)', weight: '500' },
        { label: this.t('ui.rep.cogs'), val: pnlText.costOfSales ? this.digits(pnlText.costOfSales) : pnlText.costOfSales, bar: barW(cogs), color: 'var(--ink-2)', weight: '500' },
        { label: this.t('ui.rep.salaries'), val: pnlText.salaries ? this.digits(pnlText.salaries) : pnlText.salaries, bar: barW(payrollOut), color: 'var(--ink-2)', weight: '500' },
        { label: this.t('ui.rep.overheads'), val: pnlText.overheads ? this.digits(pnlText.overheads) : pnlText.overheads, bar: barW(overhead), color: 'var(--ink-2)', weight: '500' }
      ]
    };
    ...
    const pack = { period: this.display(reportPeriod.range || periodLabel), accountant: s.accountantName || '', monthlyOn: !!(s.toggles && s.toggles.monthlySend), items: [
      { label: this.t('ui.pack.pnl'), meta: 'PDF' },
      { label: this.t('ui.pack.ledger'), meta: this.t('ui.pack.ledgerMeta', { count: s.txns.length }) },
      { label: this.t('ui.pack.invoices'), meta: this.t('ui.pack.invMeta', { count: s.invoices.length }) },
      { label: this.t('ui.pack.tax'), meta: 'PDF' },
      { label: this.t('ui.pack.recon'), meta: this.t('ui.pack.reconMeta', { pct: this.liveMatch().percent }) }
    ] };
    ...
    const branchPerf = branchRows.map(b => Object.assign({}, b, {
      ...
      share: this.digits(Math.round((b.revenue / revTotal) * 100) + '%')
    }));

    const aiInsights = [];
    if (reportPeriod.netTrendText) {
      aiInsights.push(this.t('ui.rep.aiNet', { trend: reportPeriod.netTrendText }));
    }
    if (typeof reportPeriod.moneyInTrendPct === 'number' && reportPeriod.moneyInTrendPct !== 0) {
      aiInsights.push(this.t(
        reportPeriod.moneyInTrendPct > 0 ? 'ui.rep.aiMoneyInUp' : 'ui.rep.aiMoneyInDown',
        { pct: Math.abs(reportPeriod.moneyInTrendPct) }
      ));
    }
    if (spend.insight) {
      aiInsights.push(spend.insight);
    }
    const fc = reportPeriod.forecastSummary;
    if (fc && fc.hasProjection && fc.netMinor !== 0) {
      aiInsights.push(this.t(
        fc.positive ? 'ui.rep.aiForecastIn' : 'ui.rep.aiForecastOut',
        { amt: this.display(fc.netText) }
      ));
    }
    const runwayRow = reportPeriod.runway || {};
    if (!runwayRow.profitable && runwayRow.months) {
      aiInsights.push(this.t('ui.rep.aiRunway', { months: runwayRow.months }));
    }
    if (!aiInsights.length) {
      aiInsights.push(this.t('ui.rep.aiNoInsight'));
    }

    return {
      reports,
      reportsLabel: periodLabel,
      pnl,
      runway,
      forecast,
      spend,
  ...
  checkoutPpVals(s) {
    ...
    const fields = p.fields.map((fd, i) => ({
      ...
      locked: fd.kind === 'price' || fd.kind === 'mail' || !!fd.locked,
      removable: fd.kind !== 'price' && fd.kind !== 'mail' && !fd.locked,
      ...
      setPrice: (e) => { const v = String(e.target.value || '').replace(/[^0-9.]/g, ''); this.setState(st => ({ ppPublishError: '', pp: Object.assign({}, st.pp, { fields: st.pp.fields.map((x, j) => j === i ? Object.assign({}, x, { unitPrice: v }) : x) }) })); },
      ...
      remove: () => {
        if (fd.kind === 'price' || fd.kind === 'mail') return;
        this.setState(st => ({ ppFieldEdit: null, ppFieldDraft: null, pp: Object.assign({}, st.pp, { fields: st.pp.fields.filter((_, j) => j !== i) }) }));
      }
    }));
    ...
    return {
      ...
      priceMenu: {
        ...
        options: [
          ...
          label, sub, d,
          go: this.memo('_ppp', k, () => (e) => {
            if (e) { if (e.preventDefault) e.preventDefault(); if (e.stopPropagation) e.stopPropagation(); }
            this.setState({ ppPriceMenu: false, ppAmountMode: k, ppFieldDraft: null, ppPublishError: '' });
            clearTimeout(this._ppEditT);
            this._ppEditT = setTimeout(() => this.setState(st => {
              const idx = st.pp.fields.findIndex(x => x.kind === 'price');
              return { ppFieldEdit: idx < 0 ? null : idx };
            }), 0);
          })
        }))
      },
      ...
      total: (() => {
        const priceField = p.fields.find(x => x.kind === 'price');
        if ((s.ppAmountMode || 'fixed') === 'open') return '';
        const raw = (s.ppPayLabelEdit && s.ppPayAmountDraft != null)
          ? s.ppPayAmountDraft
          : ((priceField && priceField.unitPrice) || '');
        const major = Math.abs(parseFloat(String(raw).replace(/[^0-9.]/g, '')) || 0);
        return this.fmt(major);
      })(),
      ...
      payAmountOpen: (s.ppAmountMode || 'fixed') === 'open',
      payAmountHint: (s.ppAmountMode || 'fixed') === 'qty'
        ? 'Price for one item. Customers choose how many.'
        : 'This is what the customer pays.',
      payAmountDraft: s.ppPayAmountDraft != null ? s.ppPayAmountDraft : String(((p.fields.find(x => x.kind === 'price') || {}).unitPrice) || ''),
      editPayLabel: () => this.setState(st => ({
        ppPayLabelEdit: true,
        ppPayLabelDraft: st.pp.payLabel || 'Pay',
        ppPayAmountDraft: String((((st.pp.fields || []).find(x => x.kind === 'price') || {}).unitPrice) || '')
      })),
      setPayLabelDraft: (e) => this.setState({ ppPayLabelDraft: e.target.value }),
      setPayAmountDraft: (e) => this.setState({ ppPayAmountDraft: String(e.target.value || '').replace(/[^0-9.]/g, '') }),
      cancelPayLabel: () => this.setState({ ppPayLabelEdit: false, ppPayLabelDraft: null, ppPayAmountDraft: null }),
      savePayLabel: () => this.setState(st => ({
        ppPayLabelEdit: false, ppPayLabelDraft: null, ppPayAmountDraft: null,
        pp: Object.assign({}, st.pp, {
          payLabel: (st.ppPayLabelDraft || 'Pay'),
          fields: st.pp.fields.map(x => x.kind === 'price'
            ? Object.assign({}, x, { unitPrice: st.ppPayAmountDraft != null ? st.ppPayAmountDraft : x.unitPrice })
            : x)
        })
      })),
      statusLabel: this.statusLabel(p.published ? 'Published' : 'Draft'),
  ...
  pageVals(s, d) {
    ...
    const detType = d && d.type;
    const onPay = this.needPage('payments') || detType === 'link' || detType === 'plan' || detType === 'sub';
    const onInv = this.needPage('invoicing') || detType === 'invoice' || detType === 'client';
    const onTxn = this.needPage('transactions') || detType === 'txn' || detType === 'match';
    const onTeam = this.needPage('team') || detType === 'member';
    const onPayr = this.needPage('payroll') || detType === 'employee' || detType === 'payslip';
    const onAcc = this.needPage('accounting') || detType === 'synclog';
    const onConn = this.needPage('connections') || detType === 'gateway';

    const links = onPay ? s.links.map(l => Object.assign({}, l, {
      amt: this.fmt(l.amount), chip: this.chip(l.status), open: this.open('link', l.id),
      uses: this.digits(String(l.uses)), created: this.display(l.created),
      copy: this.memo('_lcp', l.id, () => (e) => { if (e && e.stopPropagation) e.stopPropagation(); this.copyPayUrl(l.payUrl); })
    })) : [];
    const mrrAll = onPay ? s.plans.reduce((a, p) => a + p.mrr, 0) : 0;
    const subsAll = onPay ? s.plans.reduce((a, p) => a + p.subs, 0) : 0;
    const plans = onPay ? s.plans.map(p => Object.assign({}, p, {
      copy: this.memo('_plc', p.id, () => (e) => { e.stopPropagation(); this.copyPayUrl(p.signupUrl || p.payUrl); }),
      amt: this.fmt0(p.amount) + ' / ' + this.t('pages.common.' + String(p.interval || 'Month').toLowerCase()),
      every: (s.upcomingCharges || []).some(c => c.planId === p.id)
        ? this.t('ui.subs.nextCharge', { when: this.display(((s.upcomingCharges || []).find(c => c.planId === p.id) || {}).when || '') })
        : (p.status === 'Canceled' ? this.statusLabel('Canceled') : this.t('ui.subs.noCharge')),
      mrrT: this.fmt0(p.mrr), subsT: this.digits(String(p.subs)),
      share: Math.round((p.mrr / Math.max(mrrAll, 1)) * 100) + '%',
      open: this.open('plan', p.id)
    })) : [];
    const subsSum = {
      mrr: this.fmt0(mrrAll), people: this.digits(String(subsAll)),
      plans: this.t(s.plans.length === 1 ? 'ui.subs.planOne' : 'ui.subs.planMany', { count: s.plans.length }),
      avg: this.fmt0(subsAll ? Math.round(mrrAll / subsAll) : 0)
    };
    const allInvoices = onInv ? s.invoices.map(i => Object.assign({}, i, { amt: this.fmt(i.amount), chip: this.chip(i.status), open: this.open('invoice', i.id) })) : [];
    const invoices = onInv ? this.filteredInvoices(allInvoices) : [];
    const remindable = { Sent: 1, Viewed: 1, Overdue: 1, 'Awaiting Settlement': 1 };
    const reminderInvoices = onInv ? (s.reminderInvoices && s.reminderInvoices.length ? s.reminderInvoices : allInvoices.filter(i => remindable[i.status])).map(i => Object.assign({}, i, {
      amt: this.fmt(i.amount), chip: this.chip(i.status),
      remind: this.memo('_irn', i.id, () => () => this.openReminderPreview(i))
    })) : [];
    const clients = onInv ? s.clients.map(c => Object.assign({}, c, {
    ...
    const team = onTeam ? s.team.map(m => Object.assign({}, m, { open: this.open('member', m.id), initials: this.initialsOf(m.name), role: this.roleLabel(m.role) })) : [];
    const employees = onPayr ? s.employees.map(e => Object.assign({}, e, {
    ...
    const branches = (this.needPage('reports') || detType === 'branch') ? s.branches.map(b => Object.assign({}, b, { rev: this.fmt0(b.revenue), staffT: String(b.staff), open: this.open('branch', b.id) })) : [];
    ...
    } : { any: exportHistory.length > 0, none: exportHistory.length === 0, rows: [] };
    const syncLogRows = onAcc ? (s.syncLog || []).map(l => Object.assign({}, l, { chip: this.chip(l.status), open: this.open('synclog', l.id), itemsT: String(l.items), errT: String(l.errors) })) : [];
    const syncLog = { any: syncLogRows.length > 0, none: syncLogRows.length === 0, rows: syncLogRows };
    const matches = onTxn ? s.matches.map(m => Object.assign({}, m, { amt: this.fmt(m.amount), confT: m.conf + '%', open: this.open('match', m.id), high: m.conf >= 85, low: m.conf < 85 })) : [];

    const rows = onTxn ? this.filtered().map(t => Object.assign({}, this.row(t), { chip: this.chip(t.status) })) : [];
    const tagTotals = onTxn ? s.tags.map(tg => {
    ...
    const weekBuckets = onTxn ? weekDays.map((day, i) => {
      const offset = i - 6;
    ...
    if (d) {
      ...
      det.gateway = d.type === 'gateway'; det.report = d.type === 'report'; det.member = d.type === 'member'; det.branch = d.type === 'branch'; det.employee = d.type === 'employee'; det.payslip = d.type === 'payslip';
      const find = (arr, id) => arr.find(x => x.id === id) || {};
      if (det.txn) {
        const t = find(s.txns, d.id);
        det.o = Object.assign({}, t, {
          amt: this.fmt(t.amount),
          chip: this.chip(t.status),
          isIn: t.amount > 0,
          matchLabel: t.matchLabel || '—',
          matchLog: Array.isArray(t.matchLog) ? t.matchLog : []
        });
      }
      if (det.invoice) {
        const i = find(s.invoices, d.id);
        const discMajor = Number(i.discount) || 0;
        const lineRows = (i.lines && i.lines.length)
          ? i.lines.map(line => {
              const qty = Number(line.quantity) || 1;
              const unitMajor = (Number(line.unitMinor) || 0) / 100;
              const majorAmt = qty * unitMajor;
              return {
                label: line.description || 'Untitled line',
                qty: String(qty),
                unit: this.plainEn(unitMajor),
                amt: this.plainEn(majorAmt),
                note: line.note || ''
              };
            })
          : [{
              label: 'Invoice',
              qty: '1',
              unit: this.plainEn((Number(i.amount) || 0) + discMajor),
              amt: this.plainEn((Number(i.amount) || 0) + discMajor)
            }];
        const lineMajor = (i.lines && i.lines.length)
          ? i.lines.reduce((sum, line) => sum + ((Number(line.quantity) || 1) * (Number(line.unitMinor) || 0)) / 100, 0)
          : (Number(i.amount) || 0) + discMajor;
        const profile = s.profile || {};
        const banks = s.banks || [];
        const payBank = banks.find(b => !b.sample) || banks[0] || {};
        det.o = Object.assign({}, i, {
          amt: this.fmt(i.amount), chip: this.chip(i.status), taxAmt: this.fmt(0), total: this.fmtEn(i.amount),
          createdOn: i.issued || i.createdOn || '',
          sentOn: i.sentOn || '—',
          viewedOn: i.viewedOn || '—',
          businessName: s.merchantName || profile.businessName || '',
          sellerAddress: profile.address || '',
          taxReg: String(profile.taxRegistrationNumber || '').trim(),
          crNumber: String(profile.crNumber || '').trim(),
          sellerPhone: String(profile.phone || '').trim(),
          sellerEmail: String(profile.email || '').trim(),
          clientAddress: (() => {
            const clients = s.clients || [];
            const buyer = clients.find(c => c.id && c.id === i.clientId)
              || clients.find(c => c.name && c.name === i.client)
              || {};
            return String(buyer.address || '').trim();
          })(),
          issuedLabel: i.issued || '',
          previewLines: lineRows,
          previewSubtotal: this.fmtEn(lineMajor),
          previewDiscount: discMajor > 0 ? this.fmtEn(discMajor) : '',
          partialPayment: !!i.partialPayment,
          notes: i.notes || '',
          termsAndConditions: i.termsAndConditions || '',
          reference: i.reference || '',
          bankName: String(profile.bankName || payBank.bank || '').trim(),
          accountName: String(profile.accountName || s.merchantName || profile.businessName || '').trim(),
          iban: String(profile.iban || '').trim(),
          accountNumber: String(profile.accountNumber || '').trim(),
          swiftCode: String(profile.swiftCode || '').trim(),
          payUrl: ((typeof location !== 'undefined' && location.origin) ? location.origin : '') + this.invoiceShareUrl(i.no),
          attachments: Array.isArray(i.attachments) ? i.attachments : [],
          sendReminder: this.memo('_irs', i.id, () => () => this.openReminderPreview(i)),
          ...
          downloadPdf: this.memo('_ipdf', i.id, () => () => this.downloadCurrentInvoicePdf(i.no, i.client)),
          printInvoice: this.memo('_ipr', i.id, () => () => this.printCurrentInvoice()),
          pdfBusy: !!s.invoicePdfBusy,
          pdfBusyOn: !!s.invoicePdfBusy
        });
      }
      if (det.link) {
        ...
        det.o = Object.assign({}, l, {
          ...
          usesT: this.digits(String(l.uses)),
      ...
      if (det.client) {
        const c = find(s.clients, d.id);
        det.o = Object.assign({}, c, {
          totalT: this.fmt0(c.total),
          edit: this.memo('_ecli', c.id, () => () => this.setState(st => ({
            modal: 'client',
            editClient: c.id,
            form: Object.assign({}, st.form, {
              cliName: c.name || '',
              cliEmail: c.email || '',
              cliPhone: c.phone || '',
              cliAddress: c.address || ''
            })
          })))
        });
        det.invoices = invoices.filter(i => i.client === c.name);
      }
      ...
      if (det.member) { const m = find(s.team, d.id); det.o = Object.assign({}, m, { initials: this.initialsOf(m.name), role: this.roleLabel(m.role) }); }
      if (det.branch) {
      ...
      if (det.employee) {
        const e = find(s.employees, d.id);
        const rate = s.figures && s.figures.deductionRate != null ? s.figures.deductionRate : 0;
        const ded = Math.round(e.salary * rate);
        const empSlips = (s.payslips || []).filter(p => p.employeeId === e.id).map(p => ({
          id: p.id,
          period: p.period,
          netT: this.fmt0(p.net),
          open: this.open('payslip', p.id)
        }));
        det.o = Object.assign({}, e, {
          sal: this.fmt0(e.salary),
          net: this.fmt0(e.salary - ded),
          ded: this.fmt0(ded),
          slips: empSlips,
          hasSlips: empSlips.length > 0
        });
      }
      if (det.payslip) {
        const p = find(s.payslips, d.id);
        const emp = find(s.employees, p.employeeId);
        const profile = s.profile || {};
        const pct = s.figures && s.figures.deductionRate != null ? s.figures.deductionRate : 0;
        det.o = Object.assign({}, p, {
          name: p.employeeName,
          role: p.role || emp.role || '',
          method: p.method || emp.method || 'Bank transfer',
          period: p.period,
          sal: this.fmt0(p.gross),
          ded: this.fmt0(p.deduction),
          net: this.fmt0(p.net),
          pctText: (pct * 100).toFixed(pct * 100 % 1 ? 1 : 0) + '%',
          businessName: s.merchantName || profile.businessName || '',
          sellerAddress: profile.address || '',
          ...
          accountName: profile.accountName || s.merchantName || '',
          iban: profile.iban || '',
          accountNumber: profile.accountNumber || '',
          swiftCode: profile.swiftCode || '',
          issued: (typeof FlowStore !== 'undefined' && FlowStore.formatDate)
            ? FlowStore.formatDate(p.generatedOffset || 0)
            : '',
          downloadPdf: this.memo('_spdf', p.id, () => () => this.downloadCurrentPayslipPdf(p.period, p.employeeName)),
          printSlip: this.memo('_spr', p.id, () => () => this.printCurrentInvoice()),
          pdfBusy: !!s.invoicePdfBusy
        });
      }
      det.o = det.o || {};
    ...
    const payrollTotal = (s.employees || []).reduce((a, e) => a + (Number(e.salary) || 0), 0);

    const reportVals = (s.page === 'reports' && !d) ? this.reportsPageVals(s) : REPORTS_VALS_IDLE;
    const reports = reportVals.reports;
    const pnl = reportVals.pnl;
    const runway = reportVals.runway;
    const forecast = reportVals.forecast;
    const spend = reportVals.spend;
    const ageing = reportVals.ageing;
    const pack = reportVals.pack;
    const branchPerf = reportVals.branchPerf;
    const aiInsights = reportVals.aiInsights;

    const PERM = !onTeam ? [] : ((s.rolePermissions && s.rolePermissions.length) ? s.rolePermissions.map(row => [row.area, row.owner, row.accountant, row.staff]) : [
    ...
    const pendingList = onTeam ? (s.approvalRequests || s.approvals || []) : [];
    const pending = {
    ...
    return {
      ...
      bt: { accounts: this.tabOf('bankView', 'accounts') === 'accounts', statements: this.tabOf('bankView', 'accounts') === 'statements' },
      bankViewTabs: this.tabList('bankView', [['accounts', this.t('ui.bank.tabAccounts')], ['statements', this.t('ui.bank.tabStatements')]]),
      ...
      reportsLabel: reportVals.reportsLabel,
      pnl, runway, forecast, spend, ageing, pack, branchPerf, aiInsights, perms, limits, pending,
      providers: onConn ? UPCOMING_GATEWAYS.map(p => ({
        n: p.n,
        initial: p.initials,
        s: this.t('pages.common.comingSoon'),
        note: false,
        go: () => this.toast(this.t('ui.pay.notifyToast', { name: p.n }))
      })) : [],
      history: onTeam ? s.history.filter(x => s.actType === 'all' || x.kind === s.actType).map(x => {
      ...
      actNone: onTeam ? s.history.filter(x => s.actType === 'all' || x.kind === s.actType).length === 0 : true,
      ...
      subsAll: onPay ? s.subscribers.map(x => Object.assign({}, x, { chip: this.chip(x.status), open: this.open('sub', x.id) })) : [],
      reminders: s.reminders.map(r => ({ label: this.t('ui.rem.' + r.id), on: r.on, off: !r.on, Track: SW(!!r.on), Knob: KN(!!r.on), go: this.memo('_rmd', r.id, () => () => this.setState(st => ({ reminders: st.reminders.map(x => x.id === r.id ? Object.assign({}, x, { on: !x.on }) : x) }))) })),
      ...
      fmenu: [
        ...
        this.filterMenu('tag', ['All'].concat((s.tags && s.tags.length) ? s.tags : []), 'Tag')
      ],
      ...
      iclear: {
        show: !!((s.invSearch || '').trim() || (s.invFilter && (s.invFilter.status !== 'All' || s.invFilter.range !== 'All time'))),
        go: () => this.setState({ invSearch: '', invFilter: { status: 'All', range: 'All time' }, invFilterOpen: null })
      },
      imenu: [
        this.invFilterMenu('status', ['All', 'Draft', 'Sent', 'Viewed', 'Overdue', 'Awaiting Settlement', 'Paid', 'Refunded'], 'Status'),
        this.invFilterMenu('range', ['All time', 'Last 7 days', 'Last 30 days', 'This month'], 'Period')
      ],
      invSearch: s.invSearch || '',
      setInvSearch: this.memo('_invq', 'set', () => (e) => this.setState({ invSearch: e.target.value })),
      empt: {
        rows: rows.length === 0,
        invoices: s.invoices.length === 0,
        invFiltered: s.invoices.length > 0 && invoices.length === 0,
      ...
      counts: { rows: String(onTxn ? rows.length : s.txns.length), total: String(s.txns.length), invoices: String(s.invoices.length), invShown: String(invoices.length), matches: String(this.liveMatch().open) },
      tagList: this.needPage('settings', 'tags') ? (() => {
        const tree = this.tagTree(s);
        const ordered = [];
        tree.roots.forEach(root => {
          ordered.push({ name: root, depth: 0, parent: '' });
          (tree.children[root] || []).forEach(child => ordered.push({ name: child, depth: 1, parent: root }));
        });
        const rows = ordered.map(row => {
          ...
          return {
            ...
            depth: row.depth,
            child: row.depth > 0,
            rowStyle: row.depth > 0
              ? 'display:flex; align-items:center; flex-wrap:wrap; gap:10px; padding:10px 13px'
              : 'display:flex; align-items:center; flex-wrap:wrap; gap:10px; padding:11px 13px; cursor:pointer',
            count: String(count),
            countLabel: this.t('settings.tags.items', { count }),
            parent: row.parent,
            parentLabel: row.parent
              ? this.t('settings.tags.under', { parent: row.parent })
              : this.t('settings.tags.topLevel'),
            open: this.memo('_to', t, () => () => this.setState({ openTag: t })),
            edit: this.memo('_te', t, () => () => this.setState(st => {
              const liveParents = st.tagParents || {};
              return {
                modal: 'tag',
                editTag: t,
                form: Object.assign({}, st.form, {
                  newTag: t,
                  newTagParent: liveParents[t] || ''
                })
              };
            })),
            del: this.memo('_dt', t, () => () => {
              if (typeof FlowStore === 'undefined' || !FlowStore.removeTag) { this.toast(this.t('chrome.toast.tagRemoveFailed')); return; }
              try {
                FlowStore.removeTag(t);
                this.applyStore({ openTag: this.state.openTag === t ? null : this.state.openTag });
                this.toast(this.t('chrome.toast.tagRemoved'));
              } catch (err) {
                this.toast((err && err.message) || this.t('chrome.toast.tagRemoveFailed'));
              }
            })
          };
        });
        rows.forEach(row => {
          row.subs = row.child ? [] : rows.filter(child => child.parent === row.id);
        });
        return rows;
      })() : [],
      newTag: this.needPage('settings', 'tags') ? (() => {
        const catalog = (s.tags && s.tags.length) ? s.tags : [];
        const parentsMap = s.tagParents || {};
        const roots = catalog.filter(t => {
          const parent = parentsMap[t];
          return !parent || catalog.indexOf(parent) < 0;
        });
        const editing = s.editTag || '';
        const childCount = editing
          ? catalog.filter(name => parentsMap[name] === editing).length
          : 0;
        const parentChoices = [{ value: '', label: this.t('settings.tags.noParent') }].concat(
          (editing && childCount
            ? []
            : roots.filter(name => name !== editing)
          ).map(name => ({ value: name, label: name }))
        );
        return {
          name: (s.form && s.form.newTag) || '',
          parent: (s.form && s.form.newTagParent) || '',
          showParent: !!s.editTag,
          setName: (e) => this.setState(st => ({ form: Object.assign({}, st.form, { newTag: e.target.value }) })),
          setParent: (e) => this.setState(st => ({ form: Object.assign({}, st.form, { newTagParent: e.target.value }) })),
          parents: parentChoices,
          open: this.memo('_ntag', 'root', () => () => this.setState(st => ({
            modal: 'tag',
            editTag: null,
            form: Object.assign({}, st.form, { newTag: '', newTagParent: '' })
          })))
        };
      })() : { name: '', parent: '', showParent: false, setName: noop, setParent: noop, parents: [], open: noop },
      tagFolder: this.needPage('settings', 'tags') ? (() => {
        const name = s.openTag || '';
        const tree = this.tagTree(s);
        const on = !!(name && tree.roots.indexOf(name) >= 0);
        const kids = on ? (tree.children[name] || []) : [];
        return {
          on,
          name,
          back: this.memo('_tback', 'x', () => () => this.setState({ openTag: null })),
          add: this.memo('_tadd', name, () => () => this.setState(st => ({
            modal: 'tag',
            editTag: null,
            form: Object.assign({}, st.form, { newTag: '', newTagParent: st.openTag || '' })
          }))),
          empty: kids.length === 0
        };
      })() : { on: false, name: '', back: noop, add: noop, empty: true },
      rec: this.needPage('invoicing', 'recurring') ? (() => {
      ...
      linkSum: onPay ? (() => {
        ...
        return {
          ...
          active: this.digits(String(s.links.filter(l => l.status === 'Active').length)),
          uses: this.digits(String(paidUses))
        };
      })() : { total: '', active: this.digits('0'), uses: this.digits('0') },
      invSum: (() => {
      ...
      co: s.checkout,
      payroll: onPayr ? (() => {
        const gross = payrollTotal;
        const pct = s.figures && s.figures.deductionRate != null ? s.figures.deductionRate : 0;
        const ded = Math.round(gross * pct);
        const net = gross - ded;
        ...
        const slips = (s.payslips || []).filter(p => p.period === period).map(p => ({
          id: p.id,
          name: p.employeeName,
          role: p.role || '',
          grossT: this.fmt0(p.gross),
          dedT: this.fmt0(p.deduction),
          netT: this.fmt0(p.net),
          open: this.open('payslip', p.id)
        }));
        return {
          total: this.fmt0(gross), net: this.fmt0(net), count: this.digits(String((s.employees || []).length)), deductions: this.fmt0(ded), pctText: (pct * 100).toFixed(pct * 100 % 1 ? 1 : 0) + '%',
          period,
          empsLabel: this.t('ui.payr.allEmps', { count: (s.employees || []).length }),
          setPeriod: (e) => this.setState(st => ({ form: Object.assign({}, st.form, { payrollPeriod: e.target.value }) })),
          alreadyPosted: already,
          canPost: !already,
          postLabel: already ? this.t('ui.payr.alreadyPosted', { period: this.display(period) }) : this.t('ui.payr.postToTxns'),
          postStyle: already ? baseStyle + '; opacity:.55; pointer-events:none' : baseStyle,
          post: already ? noop : () => this.postPayrollToLedger(),
          slips,
          hasSlips: slips.length > 0,
          slipEmpty: slips.length === 0
        };
      })() : { total: '', net: '', count: '0', deductions: '', pctText: '', period: '', empsLabel: '', alreadyPosted: false, canPost: false, postLabel: '', postStyle: '', setPeriod: noop, post: noop, slips: [], hasSlips: false, slipEmpty: true },
      connStats: (() => {
        const all = ['skipcash', 'bank', 'zoho', 'tally', 'shopify'];
        const active = all.filter(k => k === 'tally' || s.conns[k]).length;
        return { active: this.digits(String(active)), attention: '', who: '', show: false };
      })(),
      connMan: {
      ...
      smart: (() => {
        ...
        return {
          ...
          note: this.t('ui.smart.sampleNote'),
          steps: (analytics.steps || []).map(step => ({
            label: step.label === 'Opened checkout' ? this.t('ui.smart.opened')
              : step.label === 'Entered details' ? this.t('ui.smart.details')
              : step.label === 'Paid' ? this.t('ui.smart.paid')
              : step.label,
            pct: this.digits(String(step.percent) + '%'),
            bar: step.percent + '%'
          }))
        };
      })(),
      shopify: (() => {
        ...
        return {
          ...
          shopLine: sh.connected ? sh.shopDomain : this.t('ui.shop.tagOrders'),
          appsLine: sh.connected ? this.t('ui.shop.ordersOnLedger', { count: sh.orderCount || 0 }) : this.t('ui.shop.tagOrders'),
          ...
          orderCount: this.digits(String(sh.orderCount || 0)),
          plugin: sh.connected
            ? this.t('pages.common.connected') + ' · ' + this.t('pages.common.simulated')
            : this.t('pages.common.notConnected'),
          syncNote: sh.connected
            ? this.t(sh.sampleReady ? 'ui.shop.syncReady' : 'ui.shop.syncLive')
            : this.t('ui.shop.syncHist'),
          begin: () => {
      ...
      bankOn: {
        upload: this.openModal('statement'),
        demo: this.openModal('bankDemo'),
        soonChip: this.chip('Coming soon'),
        soonBtn: 'flex:0 0 auto; font-size:13px; font-weight:650; padding:10px 18px; border-radius:10px; border:1px solid var(--line); background:var(--btn-light); color:var(--ink-4); opacity:.55; cursor:default'
      },
  ...
  renderVals() {
    ...
    const NAV = [
      ['dashboard', 'chrome.nav.dashboard',
      ...
      ['connections', 'chrome.nav.connections',
      ...
      ['reports', 'chrome.nav.reports',
        'M4.6 16.6v-4.4M9.3 16.6V9M17.4 16.6V5.4',
        'M14 16.6V7.4'],
      ['team', 'chrome.nav.team',
    ...
    const nav = NAV.map(([key, labelKey, dpath, fpath]) => {
      const on = s.page === key || s.fly === key;
      return {
        key, label: this.t(labelKey), d: dpath + ' ' + fpath, go: this.go(key, this.dflt(key)), ic: on ? 'var(--accent)' : 'var(--ink-4)',
        badge: '',
    ...
    Object.keys(DEST).forEach(k => {
      TABS[k] = (HUBKEYS.indexOf(k) >= 0 ? [['hub', this.t('chrome.mobile.overview')]] : []).concat(DEST[k].map(e => [e[0], e[1]]));
    });
    ...
    const quick = [
      [this.t('overview.quick.sendLink'), 'link',
      ...
      [this.t('overview.quick.scanBill'), 'scan',
        ...
        'M5.6 10h8.8'],
      [this.t('overview.quick.logExpense'), 'expense',
        'M10 15.6V5.2M5.8 9.4 10 5.2l4.2 4.2',
        'M4.4 16.8h11.2'],
      [this.t('overview.quick.createInvoice'), 'invoice',
        'M6.1 2.8h7.8v14.4l-1.95-1.3-1.95 1.3-1.95-1.3L6.1 17.2ZM8.4 9.6h3.2M8.4 12.4h2.2',
        'M6.1 6.4h7.8']
    ].map(([label, kind, dp, fp]) => ({ label, d: dp + ' ' + fp, go: kind === 'invoice' ? this.go('invoicing', 'create') : this.openModal(kind) }));
    ...
    const kpis = [
      { label: this.t('overview.kpi.moneyIn'), val: period.moneyInText ? this.digits(period.moneyInText) : this.fmt0(inflow), delta: this.digits(period.moneyInShare || ''), up: period.moneyInTrendPct > 0, down: period.moneyInTrendPct < 0, d: 'M10 15.4V6.2M6.2 10 10 6.2l3.8 3.8M10 2.2a7.8 7.8 0 1 1 0 15.6 7.8 7.8 0 0 1 0-15.6Z' },
      { label: this.t('overview.kpi.moneyOut'), val: period.moneyOutText ? this.digits(period.moneyOutText) : this.fmt0(Math.abs(outflow)), delta: this.digits(period.moneyOutShare || ''), up: period.moneyOutTrendPct > 0, down: period.moneyOutTrendPct < 0, d: 'M10 4.6v9.2M6.2 10l3.8 3.8L13.8 10M10 2.2a7.8 7.8 0 1 1 0 15.6 7.8 7.8 0 0 1 0-15.6Z' },
      { label: this.t('overview.kpi.outstanding'), val: this.fmt0(outstanding), delta: this.t('overview.kpi.invoices', { count: unpaidN }), up: false, down: false, d: 'M6.1 2.8h7.8v14.4l-1.95-1.3-1.95 1.3-1.95-1.3L6.1 17.2ZM6.1 6.4h7.8' },
      { label: this.t('overview.kpi.autoMatched'), val: this.digits(matchPct + '%'), delta: this.t('overview.kpi.open', { count: openN }), up: false, down: false, d: 'm6 10.2 2.9 2.9L14.4 7.6M10 2.2a7.8 7.8 0 1 1 0 15.6 7.8 7.8 0 0 1 0-15.6Z' }
    ].map(k => Object.assign({}, k, { neutral: !k.up && !k.down && !!k.delta }));

    const MODAL = {
      pageSettings: [this.t('pages.modal.pageSettings'), this.t('pages.modal.saveSettings')],
      receipts: [this.t('pages.modal.receipts'), this.t('pages.modal.save')],
      link: [this.t('pages.modal.link'), this.t('pages.modal.createLink')],
      scan: [this.t('pages.modal.scan'), this.t('pages.modal.saveTxn')],
      expense: [this.t('pages.modal.expense'), this.t('pages.modal.logExpense')],
      invoice: [this.t('pages.modal.invoice'), this.t('pages.modal.createInvoice')],
      plan: [this.t('pages.modal.plan'), this.t('pages.modal.createPlan')],
      member: [this.t('pages.modal.member'), this.t('pages.modal.sendInvite')],
      employee: [this.t('pages.modal.employee'), this.t('pages.modal.addEmployee')],
      client: [this.t('pages.modal.client'), this.t('pages.modal.addClient')],
      tag: [this.t('pages.modal.tag'), this.t('pages.modal.createTag')],
      help: [this.t('pages.modal.help'), this.t('pages.modal.gotIt')],
      reset: [this.t('settings.reset.title'), this.t('settings.reset.cta')],
      removeBank: [this.t('pages.modal.removeBank'), this.t('pages.modal.removeSample')],
      statement: [this.t('pages.modal.statement'), this.t('pages.modal.importStatement')],
      bankDemo: [this.t('ui.bank.demoTitle'), ''],
      reminderPreview: [this.t('ui.rem.previewTitle'), this.t('ui.rem.previewSend')]
    };
    const demoStep = s.bankDemoStep || 0;
    const md = (s.modal === 'employee' && s.editEmp)
      ? [this.t('pages.modal.editEmployee'), this.t('pages.modal.saveChanges')]
      : (s.modal === 'client' && s.editClient)
        ? [this.t('pages.modal.editClient'), this.t('pages.modal.saveChanges')]
        : (s.modal === 'tag' && s.editTag)
          ? [this.t('pages.modal.editTag'), this.t('pages.modal.saveChanges')]
        : (s.modal === 'tag' && s.openTag)
          ? [this.t('pages.modal.subTag'), this.t('pages.modal.createTag')]
        : (s.modal === 'bankDemo')
          ? (demoStep === 4
            ? [this.t('ui.bank.demoStep5Title'), this.t('pages.modal.gotIt')]
            : demoStep === 3
              ? [this.t('ui.bank.demoStep4Title'), this.t('ui.bank.demoLinkAccounts')]
              : demoStep === 2
                ? [this.t('ui.bank.demoStep3Title'), this.t('chrome.modal.continue')]
                : demoStep === 1
                  ? [this.t('ui.bank.demoStep2Title'), this.t('chrome.modal.continue')]
                  : [this.t('ui.bank.demoStep1Title'), this.t('chrome.modal.continue')])
        : (MODAL[s.modal] || ['', '']);

    const vals = {
      areaKey: s.page + ':' + this.tabOf(s.page, this.dflt(s.page)) + ':' + (d || '') + ':' + (ppFocus ? 'pp' : ''),
      kpis,
      ...
      sty: {
        ...
        contentInner: ppFocus ? 'width:100%' : 'max-width:1180px; margin:0 auto',
        ...
        grid2b: mob ? 'display:grid; grid-template-columns:minmax(0,1fr); gap:16px; margin-top:16px; align-items:start' : 'display:grid; grid-template-columns:minmax(0,1.55fr) minmax(0,1fr); gap:22px; margin-top:22px; align-items:start',
      ...
      bottomNav: [
        ['dashboard', this.t('chrome.mobile.home'), 'M12.8 3h2.6A1.6 1.6 0 0 1 17 4.6v2.6a1.6 1.6 0 0 1-1.6 1.6h-2.6a1.6 1.6 0 0 1-1.6-1.6V4.6A1.6 1.6 0 0 1 12.8 3ZM4.6 11.2h2.6a1.6 1.6 0 0 1 1.6 1.6v2.6A1.6 1.6 0 0 1 7.2 17H4.6A1.6 1.6 0 0 1 3 15.4v-2.6a1.6 1.6 0 0 1 1.6-1.6Z', 'M4.6 3h2.6a1.6 1.6 0 0 1 1.6 1.6v2.6A1.6 1.6 0 0 1 7.2 8.8H4.6A1.6 1.6 0 0 1 3 7.2V4.6A1.6 1.6 0 0 1 4.6 3Z'],
        ['transactions', this.t('chrome.mobile.activity'), 'M13.9 3.4 17 6.5l-3.1 3.1M17 6.5H6.5a2.9 2.9 0 0 0-2.9 2.9M6.1 16.6 3 13.5l3.1-3.1M3 13.5h10.5a2.9 2.9 0 0 0 2.9-2.9', 'M10 12.6a1.1 1.1 0 1 1 0 2.2 1.1 1.1 0 0 1 0-2.2Z'],
        ['invoicing', this.t('chrome.mobile.invoices'), 'M6.1 2.8h7.8v14.4l-1.95-1.3-1.95 1.3-1.95-1.3L6.1 17.2ZM8.4 9.6h3.2M8.4 12.4h2.2', 'M6.1 6.4h7.8'],
        ['payments', this.t('chrome.mobile.pay'), 'M4 5.2h12a1.8 1.8 0 0 1 1.8 1.8v6a1.8 1.8 0 0 1-1.8 1.8H4a1.8 1.8 0 0 1-1.8-1.8V7A1.8 1.8 0 0 1 4 5.2ZM5.2 12.2h3.2', 'M2.2 8.6h15.6']
        ...
        key: 'more', short: this.t('chrome.mobile.more'),
      ...
      acct: (() => {
        ...
        return {
          ...
          pwLabel: s.acctPwShow ? this.t('settings.account.hide') : this.t('settings.account.show'),
          hint: !pw ? this.t('settings.account.hintKeep') : strong ? this.t('settings.account.hintStrong') : this.t('settings.account.hintShort'),
          ...
          save: () => {
            if (!/.+@.+\..+/.test(s.acctEmail)) { this.toast(this.t('chrome.toast.emailInvalid')); return; }
            if (pw && pw.length < 8) { this.toast(this.t('chrome.toast.passwordShort')); return; }
            this.setState({ acctPw: '', acctPwShow: false });
            this.toast(pw ? this.t('chrome.toast.accountPasswordUpdated') : this.t('chrome.toast.accountUpdated'));
          }
        };
      })(),
      notif: [
        ['payments', this.t('settings.account.notifPayments')],
        ['overdue', this.t('settings.account.notifOverdue')],
        ['sync', this.t('settings.account.notifSync')]
      ].map(([k, label]) => {
        const cur = (s.notifPrefs || {})[k] || 'both';
        return {
          label,
          options: [['both', this.t('settings.account.notifBoth')], ['email', this.t('settings.account.notifEmail')], ['app', this.t('settings.account.notifApp')], ['off', this.t('settings.account.notifOff')]].map(([v, ol]) => ({
            label: ol,
      ...
      backNav: (() => {
        ...
        return {
          ...
          label: this.t('pages.hub.backOverview'),
      ...
      hub: (() => {
        ...
        const { D, HUBS } = this.dests();
        const META = {
          transactions: { matching: this.t('pages.hub.needLook', { count: this.liveMatch().open }) },
          invoicing: { all: this.t('pages.hub.unpaid', { count: (s.invoiceTotals && s.invoiceTotals.outstandingCount) || 0 }) },
          team: { members: this.t('pages.hub.people', { count: s.team.length }) },
          payroll: { employees: this.t('pages.hub.onPayroll', { count: s.employees.length }) },
          reports: { branches: this.t('pages.hub.branches', { count: s.branches.length }) }
        };
        ...
        return {
          ...
          cards: list.map(([tab, title, desc, ic]) => ({
            ...
            soonChip: (s.page === 'payments' && tab === 'bank') ? this.chip('Coming soon') : '',
            go: this.go(s.page, tab)
          })),
          back: { show: !d && list.length > 0 && HUBS.indexOf(s.page) >= 0 && !onHub && !(s.page === 'payments' && this.tabOf('payments', this.dflt('payments')) === 'checkout' && s.ppView !== 'list'), text: this.t('pages.hub.backTo', { title: (this.headFor() || {}).title || '' }), go: this.tabH(s.page, 'hub') }
        };
      })(),
      flyout: (() => {
        ...
        return {
          ...
          style: 'position:fixed; z-index:60; left:' + (s.flyLeft || 90) + 'px; top:' + top + 'px; width:288px; background:var(--modal); backdrop-filter:blur(34px) saturate(165%); -webkit-backdrop-filter:blur(34px) saturate(165%); border:1px solid var(--glass-edge); border-radius:11px; box-shadow:0 36px 80px -34px rgba(0,0,0,.55); overflow:hidden',
          rows: rows.map(([tab, title, desc, ic]) => {
            const cur = s.page === k && this.tabOf(k, this.dflt(k)) === tab;
            return {
            short: title,
            ic: cur ? 'var(--accent)' : 'var(--ink-4)',
            style: 'width:100%; display:flex; align-items:center; gap:12px; padding:11px 12px; border-radius:11px; font-size:13.5px; letter-spacing:-.01em; transition:background .14s ease, color .14s ease; font-weight:' + (cur ? '650' : '500') + '; color:' + (cur ? 'var(--accent)' : 'var(--ink-2)') + '; background:' + (cur ? 'var(--accent-soft)' : 'transparent') + '; text-align:left',
            title, desc, d: IC[ic] || '',
          ...
          close: (e) => {
            ...
            if (to && to.closest && (to.closest('aside') || to.closest('.flow-open-menu'))) return;
            clearTimeout(this._flyT);
            this.setState({ fly: null });
          }
        };
      })(),
      env: (() => {
        ...
        return {
          ...
          note: live ? this.t('ui.pay.liveNote') : this.t('ui.pay.sandboxNote')
        };
      })(),
      envLabel: this.t('chrome.sandbox'),
      sandboxTip: !!s.sandboxTip,
      sandboxTipOn: () => this.setState({ sandboxTip: true }),
      sandboxTipOff: () => this.setState({ sandboxTip: false }),
      pp: (s.page === 'payments' && !d && this.tabOf('payments', this.dflt('payments')) === 'checkout' && (s.ppView === 'edit' || s.ppView === 'published'))
        ? this.checkoutPpVals(s)
        : PP_VALS_IDLE,
      ps: (() => {
        const p = s.ps;
        ...
        return {
          ...
          setCloseDate: e => this.setState(st => ({ ps: Object.assign({}, st.ps, { closeDate: e.target.value }) })),
          afterMsg: opt('after', 'message'), afterRedirect: opt('after', 'redirect'),
          redirectOn: p.after === 'redirect', msgOn: p.after === 'message',
          redirectUrl: p.redirectUrl || '',
          setRedirectUrl: e => this.setState(st => ({ ps: Object.assign({}, st.ps, { redirectUrl: e.target.value }) }))
        };
      })(),
      rc: (() => {
        const r = s.rc;
      ...
      nv: this.needPage('invoicing', 'create') ? (() => {
        const iv = s.nv || {};
        ...
        const disc = Math.max(0, num(iv.discount));
        const taxable = Math.max(0, sub - disc);
        const taxAmt = taxable * taxPct / 100;
        const totalMajor = taxable + taxAmt;
        const persistLinesFrom = (rows) => rows.filter(x => x.desc || num(x.price)).map(x => ({
        ...
        const saveInvoice = (draft) => {
          flushNvDrafts();
          const live = this.state.nv || {};
          const liveItems = live.items || [{ desc: '', qty: '1', price: '' }];
          const liveClient = live.client || '';
          const liveSub = liveItems.reduce((a, x) => a + num(x.qty) * num(x.price), 0);
          const liveDisc = Math.max(0, num(live.discount));
          const liveTaxPct = num(live.tax != null ? live.tax : 0);
          const liveTaxable = Math.max(0, liveSub - liveDisc);
          const liveTotal = liveTaxable + liveTaxable * liveTaxPct / 100;
          if (!liveClient) { this.toast('Pick a client first'); return; }
          if (!liveSub) { this.toast('Add at least one line item'); return; }
          if (liveDisc > liveSub) { this.toast('Discount cannot exceed the subtotal'); return; }
          const dueRaw = live.due == null
            ? ((typeof FlowStore !== 'undefined' && FlowStore.dateInputValue) ? FlowStore.dateInputValue(14) : '')
            : live.due;
          const saved = this.persistCreatedInvoice({
            clientName: liveClient,
            amountMajor: liveTotal,
            issuedRaw: live.issued,
            dueRaw,
            draft: !!draft,
            lines: persistLinesFrom(liveItems),
            discountMinor: Math.round(liveDisc * 100),
            clientAddress: String(live.clientAddress || '').trim(),
            notes: String(live.notes || '').trim(),
            termsAndConditions: String(live.terms || '').trim(),
            reference: String(live.reference || '').trim()
          });
          if (saved.error) {
            if (saved.field === 'issued') {
              this.setState(st => ({ nv: Object.assign({}, st.nv, { issuedError: saved.error }) }));
            } else {
          ...
          const number = saved.invoice && saved.invoice.number ? saved.invoice.number : nextNo;
          this.copyPayUrl(this.invoiceShareUrl(number), true);
          this.applyStore({ nv: {}, nvClients: false, tab: Object.assign({}, this.state.tab, { invoicing: 'all' }) });
          this.toast('Invoice ' + number + ' created — link copied, ready to send');
        };
        ...
        const fallbackNo = 'INV-' + String((s.invoices.reduce((n, i) => Math.max(n, parseInt(String(i.no || '').replace(/\D/g, ''), 10) || 0), 148) + 1)).padStart(4, '0');
        const nextNo = (typeof FlowStore !== 'undefined' && typeof FlowStore.peekNextInvoiceNumber === 'function')
          ? FlowStore.peekNextInvoiceNumber()
          : fallbackNo;
        return {
          no: nextNo,
          clientLabel: clientName || this.t('ui.ph.selectClient'),
          ...
          clientOptions: (() => {
            const cache = this._clientOptionsCache;
            if (cache && cache.clients === s.clients && cache.clientName === clientName) return cache.output;
            const output = s.clients.map(c => ({
              ...
              go: this.memo('_nvc', c.id, () => () => this.setState(st => ({ nvClients: false, nv: Object.assign({}, st.nv, { client: c.name, clientAddress: c.address || (st.nv && st.nv.clientAddress) || '' }) })))
            }));
            this._clientOptionsCache = { clients: s.clients, clientName, output };
            return output;
          })(),
          newClient: iv.newClient || '',
          setNewClient: (e) => patch('newClient', e.target.value),
          newClientAddress: iv.newClientAddress || '',
          setNewClientAddress: (e) => patch('newClientAddress', e.target.value),
          addClient: () => {
            const nm = (s.nv && s.nv.newClient || '').trim();
            const addr = String((s.nv && s.nv.newClientAddress) || '').trim();
            if (!nm) { this.toast('Type a client name first'); return; }
            if (!addr) { this.toast('Add a client address'); return; }
            if (typeof FlowStore !== 'undefined' && FlowStore.addClient) {
              FlowStore.addClient({ name: nm, address: addr });
              this.applyStore({
                nvClients: false,
                nv: Object.assign({}, this.state.nv, {
                  client: nm,
                  newClient: '',
                  newClientAddress: '',
                  clientAddress: addr || (this.state.nv && this.state.nv.clientAddress) || ''
                })
              });
              this.toast(nm + ' added');
              return;
            ...
            this.setState(st => ({
              nvClients: false,
              clients: [{ id: 'c' + Date.now(), name: nm, email: '', address: addr, owes: 0, invoices: 0 }].concat(st.clients),
              nv: Object.assign({}, st.nv, {
                client: nm,
                newClient: '',
                newClientAddress: '',
                clientAddress: addr || (st.nv && st.nv.clientAddress) || ''
              })
            }));
            this.toast(nm + ' added');
          },
          items: items.map((x, i) => ({
            ...
            setDesc: (e) => {
              const v = e.target.value;
              this.setState(st => {
                const cur = (st.nv && st.nv.items) || [{ desc: '', qty: '1', price: '' }];
                return { nv: Object.assign({}, st.nv, { items: cur.map((y, j) => j === i ? Object.assign({}, y, { desc: v }) : y) }) };
              });
            },
            setQty: (e) => {
              const v = e.target.value;
              this.setState(st => {
                const cur = (st.nv && st.nv.items) || [{ desc: '', qty: '1', price: '' }];
                return { nv: Object.assign({}, st.nv, { items: cur.map((y, j) => j === i ? Object.assign({}, y, { qty: v }) : y) }) };
              });
            },
            setPrice: (e) => {
              const v = e.target.value;
              this.setState(st => {
                const cur = (st.nv && st.nv.items) || [{ desc: '', qty: '1', price: '' }];
                return { nv: Object.assign({}, st.nv, { items: cur.map((y, j) => j === i ? Object.assign({}, y, { price: v }) : y) }) };
              });
            },
            remove: this.memo('_nvr', String(i), () => () => setItems(((this.state.nv && this.state.nv.items) || items).filter((y, j) => j !== i)))
          })),
          addItem: () => setItems(items.concat([{ desc: '', qty: '1', price: '' }])),
          tax: iv.tax != null ? iv.tax : '0',
          setTax: (e) => patch('tax', e.target.value),
          taxAmt: money(taxAmt),
          issued: iv.issued == null ? ((typeof FlowStore !== 'undefined' && FlowStore.dateInputValue) ? FlowStore.dateInputValue(0) : '') : iv.issued,
          issuedError: iv.issuedError || '',
          issuedErrorOn: !!iv.issuedError,
          setIssued: (e) => this.setState(st => ({ nv: Object.assign({}, st.nv, { issued: e.target.value, issuedError: '' }) })),
          ...
          terms: iv.terms != null ? iv.terms : '',
          setTerms: (e) => patch('terms', e.target.value),
          reference: iv.reference != null ? iv.reference : '',
          setReference: (e) => patch('reference', e.target.value),
          discount: iv.discount != null ? iv.discount : '',
          setDiscount: (e) => patch('discount', e.target.value),
          discountAmt: disc > 0 ? 'QR ' + money(disc) : '',
          preview: items.filter(x => x.desc || num(x.price)).map(x => ({
            label: x.desc || 'Untitled line',
            qty: String(num(x.qty) || 1),
            unit: money(num(x.price)),
            amt: money(num(x.qty) * num(x.price)),
            note: x.note || ''
          })),
          subtotal: 'QR ' + money(sub),
          total: 'QR ' + money(totalMajor),
          businessName: s.merchantName || (s.profile && s.profile.businessName) || '',
          sellerAddress: (s.profile && s.profile.address) || '',
          taxReg: String((s.profile && s.profile.taxRegistrationNumber) || '').trim(),
          crNumber: String((s.profile && s.profile.crNumber) || '').trim(),
          sellerPhone: String((s.profile && s.profile.phone) || '').trim(),
          sellerEmail: String((s.profile && s.profile.email) || '').trim(),
          clientAddress: iv.clientAddress || '',
          setClientAddress: (e) => patch('clientAddress', e.target.value),
          bankName: String((s.profile && s.profile.bankName) || ((s.banks || []).find(b => !b.sample) || (s.banks || [])[0] || {}).bank || '').trim(),
          accountName: String((s.profile && s.profile.accountName) || s.merchantName || (s.profile && s.profile.businessName) || '').trim(),
          iban: String((s.profile && s.profile.iban) || '').trim(),
          accountNumber: String((s.profile && s.profile.accountNumber) || '').trim(),
          swiftCode: String((s.profile && s.profile.swiftCode) || '').trim(),
          payUrl: ((typeof location !== 'undefined' && location.origin) ? location.origin : '') + this.invoiceShareUrl(nextNo),
          issuedLabel: (() => {
            const raw = iv.issued == null ? ((typeof FlowStore !== 'undefined' && FlowStore.dateInputValue) ? FlowStore.dateInputValue(0) : '') : iv.issued;
            const off = this.dueOffsetFrom(raw);
            if (off != null && typeof FlowStore !== 'undefined' && FlowStore.formatDate) return FlowStore.formatDate(off);
            return raw || '';
          })(),
          dueLabel: (() => {
            const raw = iv.due == null ? ((typeof FlowStore !== 'undefined' && FlowStore.dateInputValue) ? FlowStore.dateInputValue(14) : '') : iv.due;
            const off = this.dueOffsetFrom(raw);
            if (off != null && typeof FlowStore !== 'undefined' && FlowStore.formatDate) return FlowStore.formatDate(off);
            return raw || '';
          })(),
          statusLabel: this.statusLabel('Draft'),
          statusChip: this.chip('Draft'),
          create: () => saveInvoice(false),
          draft: () => saveInvoice(true),
          canExport: !!clientName && sub > 0,
          exportOff: !(clientName && sub > 0) || !!s.invoicePdfBusy,
          pdfBusy: !!s.invoicePdfBusy,
          downloadPdf: () => {
            flushNvDrafts();
            const live = this.state.nv || {};
            ...
            const liveSub = liveItems.reduce((a, x) => a + num(x.qty) * num(x.price), 0);
            if (!live.client || !liveSub) return;
            const go = () => this.downloadCurrentInvoicePdf(nextNo, live.client);
          ...
          printInvoice: () => {
            ...
            const liveSub = liveItems.reduce((a, x) => a + num(x.qty) * num(x.price), 0);
            if (!live.client || !liveSub) return;
            const go = () => this.printCurrentInvoice();
            if (typeof requestAnimationFrame === 'function') requestAnimationFrame(() => requestAnimationFrame(go));
            else go();
          },
          copyLink: () => {
            flushNvDrafts();
            if (!(this.state.nv && this.state.nv.client)) return;
            const liveItems = (this.state.nv && this.state.nv.items) || [];
            const liveSub = liveItems.reduce((a, x) => a + num(x.qty) * num(x.price), 0);
            if (!liveSub) return;
            this.copyPayUrl(this.invoiceShareUrl(nextNo));
          }
        };
      })() : NV_VALS_IDLE,
      gatewayOptions: this.needPage('payments', 'gateway') ? UPCOMING_GATEWAYS.map(p => ({
        name: p.n,
        initials: p.initials,
        note: p.noteKey ? this.t(p.noteKey) + ' · ' + this.t('pages.common.comingSoon') : this.t('pages.common.comingSoon'),
        cta: this.t('ui.pay.notifyMe'),
        btnStyle: 'flex:0 0 auto; font-size:12px; font-weight:650; padding:8px 14px; border-radius:9px; white-space:nowrap; border:1px solid var(--line); background:var(--btn-light); color:var(--ink-3)',
        go: this.memo('_gw', p.n, () => () => this.toast(this.t('ui.pay.notifyToast', { name: p.n })))
      })) : [],
      brand: this.needPage('payments', 'gateway') ? (() => {
      ...
      })() : { swatches: [], pickerOpen: false, palette: [], hex: '', previewStyle: '' },
      ppGrid: s.vw < 900
      ...
      ppList: this.needPage('payments', 'checkout') ? {
        show: s.ppView === 'list',
        edit: s.ppView === 'edit',
        published: s.ppView === 'published',
        reopen: () => this.setState({ ppView: 'edit' }),
        count: this.t((s.checkoutPages || s.ppPages || []).length === 1 ? 'ui.pay.pagesOne' : 'ui.pay.pagesMany', { count: (s.checkoutPages || s.ppPages || []).length }),
        empty: (s.checkoutPages || s.ppPages || []).length === 0,
        create: () => this.setState({
          ppView: 'edit', ppEditing: null,
          ppAmountMode: 'fixed',
          ppFieldEdit: null, ppFieldDraft: null, ppPublishError: '',
        ...
        rows: (s.checkoutPages || s.ppPages || []).map(p => ({
          ...
          meta: (p.published === false || p.status === 'Draft') ? this.t('ui.pay.notPublished') : this.display((p.views || 0) + ' views · ' + (p.paid || 0) + ' paid · /pay/' + (p.slug || '')),
          amount: (p.amountMode === 'open' || p.amountText === 'Open')
            ? this.t('ui.pay.openAmount')
            : (p.amountText ? this.digits(p.amountText) : (p.amount ? this.fmt0(p.amount) : '—')),
          open: () => this.setState({
            ppView: 'edit', ppEditing: p.id,
            ppAmountMode: p.amountMode === 'open' || p.amountMode === 'qty' ? p.amountMode : 'fixed',
            ppFieldEdit: null, ppFieldDraft: null, ppPublishError: '',
            ...
            ps: {
              ...
              expiry: p.closeMode === 'date' ? 'date' : 'none',
              after: p.afterPay === 'redirect' ? 'redirect' : 'message',
              closeDate: p.closeLabel || '',
              redirectUrl: p.redirectUrl || ''
            },
            rc: {
              auto: p.receiptAuto !== false,
              showCustomer: !!p.receiptCustomer,
              ref: !!p.receiptRef
            },
            pp: { title: p.title, desc: p.desc, email: p.email, phone: p.phone, terms: p.terms,
                  published: p.published !== false, fields: (p.fields || [{ label: 'Amount', kind: 'price', locked: true, unitPrice: String(p.amount || '') }]).map(x => {
                    const next = Object.assign({}, x);
                    if (x.kind === 'price' || x.kind === 'mail') next.locked = true;
                    if (x.kind === 'price' && (next.unitPrice == null || next.unitPrice === '') && p.amount && p.amountMode !== 'open') next.unitPrice = String(p.amount);
                    return next;
                  }) }
          })
        }))
      } : { show: s.ppView === 'list', edit: s.ppView === 'edit', published: s.ppView === 'published', count: this.t('ui.pay.pagesMany', { count: 0 }), empty: true, rows: [], create: noop, back: noop, reopen: noop },
      rcSummary: s.rc.auto
        ? { title: this.t('ui.rc.sumAuto'), sub: this.t(s.rc.showCustomer ? 'ui.rc.sumAutoSubCust' : 'ui.rc.sumAutoSub') }
        : { title: this.t('ui.rc.sumManual'), sub: this.t('ui.rc.sumManualSub') },
      psSummary: s.ps.after === 'redirect'
        ? { title: this.t('ui.ps.sumRedirect'), sub: this.t(s.ps.expiry === 'date' ? 'ui.ps.sumThemeDate' : 'ui.ps.sumTheme', { theme: this.t(s.ps.theme === 'dark' ? 'ui.ps.dark' : 'ui.ps.light') }) }
        : { title: this.t('ui.ps.sumThanks'), sub: this.t(s.ps.expiry === 'date' ? 'ui.ps.sumThemeDate' : 'ui.ps.sumThemeOpen', { theme: this.t(s.ps.theme === 'dark' ? 'ui.ps.dark' : 'ui.ps.light') }) },
      navGroups: (() => {
        const by = {}; nav.forEach(n => by[n.key] = n);
        const G = [
          ['', ['dashboard']],
          [this.t('chrome.navGroup.paid'), ['payments', 'transactions', 'invoicing', 'accounting']],
          [this.t('chrome.navGroup.business'), ['reports', 'team', 'payroll']],
          [this.t('chrome.navGroup.account'), ['connections', 'settings']]
        ];
        const SEC = this._SEC || {}, IC = this._IC || {};
        nav.forEach(n => {
          ...
          n.enter = (e) => {
            ...
            if (rows.length) {
              this._flyT = setTimeout(() => {
                const left = this.state.railOpen ? 252 : 74;
                this.setState({ fly: n.key, flyTop: r.top, flyLeft: left });
              }, 140);
            } else this.setState({ fly: null });
          };
          n.leave = (e) => {
            const to = e && e.relatedTarget;
            if (to && to.closest && to.closest('.flow-open-menu')) return;
            clearTimeout(this._flyT);
            this._flyT = setTimeout(() => this.setState({ fly: null }), 80);
          };
        });
      ...
      aiChatMessages: s.aiChatMessages || [],
      aiChatEmpty: !(s.aiChatMessages || []).length,
      aiChatThinking: !!s.aiChatThinking,
      searchIdle: !!s.searchOpen && (s.search || '').trim().length === 0,
      results: (() => {
        const q = (s.search || '').trim().toLowerCase();
        const cache = this._searchResultsCache;
        if (cache && cache.q === q && cache.txns === s.txns && cache.invoices === s.invoices && cache.clients === s.clients && cache.locale === locale) {
          return cache.output;
        }
        if (!q) {
          const output = { groups: [], none: false };
          this._searchResultsCache = { q, txns: s.txns, invoices: s.invoices, clients: s.clients, locale, output };
          return output;
        }
        ...
        const iv = s.invoices.filter(i => hit(i.client) || hit(i.no) || hit(i.status) || hit(this.statusLabel(i.status))).slice(0, 4).map(i => ({
          abbr: 'IN', label: i.no + ' · ' + i.client, meta: this.statusLabel(i.status),
        ...
        const pg = NAV.filter(([k, labelKey]) => hit(this.t(labelKey)) || hit(enT(labelKey))).slice(0, 3).map(([k, labelKey]) => ({
          abbr: 'GO', label: this.t(labelKey), meta: this.t('pages.search.page'),
          go: this.memo('_sr', 'pg' + k, () => () => this.setState({ search: '', searchOpen: false, detail: null, page: k }))
        }));
        const groups = [];
        if (tx.length) groups.push({ label: this.t('pages.search.transactions'), items: tx });
        if (iv.length) groups.push({ label: this.t('pages.search.invoices'), items: iv });
        if (cl.length) groups.push({ label: this.t('pages.search.clients'), items: cl });
        if (pg.length) groups.push({ label: this.t('pages.search.jump'), items: pg });
        const output = { groups, none: groups.length === 0 };
        this._searchResultsCache = { q, txns: s.txns, invoices: s.invoices, clients: s.clients, locale, output };
        return output;
      })(),
      theme: s.theme, isDark: s.theme === 'dark', isLight: s.theme !== 'dark',
      locale, dir: 'ltr', isRtl: false, localeCode: String(locale).toUpperCase(),
      fontStack: "'Urbanist','Cairo',system-ui,sans-serif",
      t: (key, vars) => this.t(key, vars),
      statusText: (st) => this.statusLabel(st),
      display: (text) => this.display(text),
      digits: (text) => this.digits(text),
      langMenu: !!s.langMenu,
      languages: enabledLocales().map(item => ({
        code: item.code, native: item.native, english: item.english, dir: item.dir,
        on: locale === item.code,
        go: () => {
          storeLocale(item.code);
          applyLocaleDir(item.code);
          this.setState({ locale: item.code, langMenu: false });
        }
      })),
      userMenu: !!s.userMenu,
      moreOpen: !!s.moreOpen,
      ...
      me: {
        initials: this.initialsOf(s.ownerName || s.acctName, 'NA'), name: s.ownerName || s.acctName, email: s.acctEmail,
        role: this.t('chrome.user.owner'), plan: (s.plan && s.plan.tier) || '', business: s.merchantName || '',
        branch: (s.branches.find(b => b.id === s.branchId) || {}).name || '',
        limit: this.t('chrome.user.noLimit'), lastLogin: '',
        links: [
          { label: this.t('chrome.user.profile'), d: 'M10 11.4a3.2 3.2 0 1 0 0-6.4 3.2 3.2 0 0 0 0 6.4ZM4.2 16.6c.9-2.6 3.1-3.8 5.8-3.8s4.9 1.2 5.8 3.8', go: () => this.setState({ userMenu: false, langMenu: false, page: 'settings', tab: Object.assign({}, s.tab, { settings: 'profile' }), detail: null }) },
          { label: this.t('chrome.user.security'), d: 'M10 2.8 4.6 5v4.4c0 3.4 2.2 6.3 5.4 7.8 3.2-1.5 5.4-4.4 5.4-7.8V5L10 2.8Z', go: () => this.setState({ userMenu: false, langMenu: false, page: 'settings', tab: Object.assign({}, s.tab, { settings: 'security' }), detail: null }) },
          { label: this.t('chrome.user.billing'), d: 'M2.8 6.6h14.4v8.2H2.8V6.6Zm0 3.2h14.4M6 12.8h3', go: () => this.setState({ userMenu: false, langMenu: false, page: 'settings', tab: Object.assign({}, s.tab, { settings: 'billing' }), detail: null }) }
        ]
      },
      mobData: mob ? (() => {
        const recent = this.sortedTxns().slice(0, 6).map(t => Object.assign({}, this.row(t), { chip: this.chip(t.status) }));
        return {
          balance: period.netText ? this.digits(period.netText) : this.fmt(period.net), inflow: this.fmt0(inflow), outflow: this.fmt0(Math.abs(outflow)),
          recent,
          actions: [
            { label: this.t('overview.mob.request'), d: 'M10 4.4v11.2M5.6 11.2 10 15.6l4.4-4.4', go: this.openModal('link') },
            { label: this.t('overview.mob.invoice'), d: 'M6.1 2.8h7.8v14.4l-1.95-1.3-1.95 1.3-1.95-1.3L6.1 17.2ZM8.4 9.6h3.2', go: this.go('invoicing', 'create') },
            { label: this.t('overview.mob.scan'), d: 'M3 7.2V4.7A1.7 1.7 0 0 1 4.7 3h2.5M12.8 3h2.5A1.7 1.7 0 0 1 17 4.7v2.5M17 12.8v2.5a1.7 1.7 0 0 1-1.7 1.7h-2.5M7.2 17H4.7A1.7 1.7 0 0 1 3 15.3v-2.5M5.6 10h8.8', go: this.openModal('scan') },
            { label: this.t('overview.mob.expense'), d: 'M10 15.6V5.2M5.8 9.4 10 5.2l4.2 4.2M4.4 16.8h11.2', go: this.openModal('expense') }
          ],
          needsMatch: this.digits(String(this.liveMatch().open)),
          openInv: String((s.invoiceTotals && s.invoiceTotals.outstandingCount) || 0),
          outstanding: this.fmt0(outstanding),
          pending: period.pendingText ? this.digits(period.pendingText) : this.fmt0(period.pending),
          pendingLine: this.t('overview.mob.pending', { amount: period.pendingText || ('QR ' + Math.abs(period.pending).toLocaleString('en-US')) }),
          delta: this.display(period.netTrendText || ''),
          allTxns: this.sortedTxns().slice(0, 16).map(t => this.row(t)),
          invoices: s.invoices.slice(0, 12).map(i => ({ no: i.no, client: i.client, status: this.statusLabel(i.status), amt: this.fmt0(i.amount), open: this.open('invoice', i.id) })),
          goTxns: this.go('transactions', 'all'), goMatch: this.go('transactions', 'matching'), goInv: this.go('invoicing', 'all')
        };
      })() : { balance: '', inflow: '', outflow: '', recent: [], actions: [], needsMatch: '0', openInv: '0', outstanding: '', pending: '', pendingLine: '', delta: '', allTxns: [], invoices: [] },
      greet: (() => {
        const hr = new Date().getHours();
        const part = hr < 12 ? 'morning' : hr < 17 ? 'afternoon' : 'evening';
        const first = (s.ownerName || this.props.ownerName || '').split(' ')[0];
        return {
          hello: this.t('overview.greet.' + part, { name: first }),
          sub: attList.length ? this.t('overview.greet.subOpen', { count: attList.length }) : this.t('overview.greet.subClear'),
          date: this.display(new Date().toLocaleDateString('en-GB', { weekday: 'long', day: 'numeric', month: 'long' }))
        };
      })(),
      usage: (() => {
        const u = s.usage || { pct: '0%', label: '', used: 0, limit: 0 };
        return Object.assign({}, u, {
          label: (u.used != null && u.limit != null)
            ? this.t('overview.usage.line', { used: u.used, limit: u.limit })
            : this.display(u.label || '')
        });
      })(),
      profile: s.profile || {},
      plan: (() => {
        const p = s.plan || { tier: '', price: '', line: '', limitLabel: '' };
        return Object.assign({}, p, {
          price: this.digits(p.price || ''),
          line: p.price ? this.t('overview.plan.line', { tier: this.t('overview.plan.tier.' + (p.tier || 'Starter')), price: p.price }) : this.display(p.line || ''),
          limitLabel: p.limit != null ? this.t('overview.plan.limit', { limit: p.limit }) : this.display(p.limitLabel || '')
        });
      })(),
      billingPlans: (s.billingPlans || []).map(bp => Object.assign({}, bp, {
        price: bp && bp.price ? this.digits(bp.price) : bp && bp.price
      })),
      billingTrialMonths: s.billingTrialMonths || 3,
      showTax: !!s.showTax,
      ownerName: s.ownerName || '',
      bank: (() => {
        const src = (typeof FlowStore !== 'undefined' && FlowStore.bankView) ? FlowStore.bankView() : (s.bank || {});
        const decorate = (row) => {
          if (!row) return row;
          const rem = row.reminder || {};
          const status = rem.status;
          const reminderOn = !!status;
          const chipKey = status === 'current' ? 'Up to date' : status === 'due-soon' ? 'Statement due soon' : status === 'overdue' ? 'Overdue' : status === 'never' ? 'Never uploaded' : '';
          const reminderLabel = status === 'never'
            ? this.t('ui.bank.reminderNever')
            : status === 'overdue'
              ? this.t('ui.bank.reminderOverdue', { days: row.daysSince })
              : status === 'due-soon'
                ? this.t('ui.bank.reminderDueSoon', { date: row.lastUpload })
                : status === 'current'
                  ? this.t('ui.bank.reminderCurrent', { date: row.lastUpload })
                  : (rem.label || '');
          const historyLine = row.historyOn
            ? this.t('ui.bank.importHistory', { count: row.importCount, date: row.lastImportDate, rows: row.lastImportedRows })
            : '';
          return Object.assign({}, row, {
            note: /Sample data/.test(row.note || '') ? this.t('ui.bank.sample') : (row.note ? this.display(row.note) : row.note),
            reminderOn,
            reminderChip: reminderOn ? this.chip(chipKey) : '',
            reminderLabel,
            historyLine
          });
        };
        const decorated = decorate(src) || {};
        return Object.assign({}, decorated, {
          activity: src.activity ? this.digits(src.activity) : src.activity,
          extra: (src.extra || []).map(decorate)
        });
      })(),
      banks: (s.banks || []).map(row => {
        const rem = row.reminder || {};
        const status = rem.status;
        const reminderOn = !!status;
        const chipKey = status === 'current' ? 'Up to date' : status === 'due-soon' ? 'Statement due soon' : status === 'overdue' ? 'Overdue' : status === 'never' ? 'Never uploaded' : '';
        ...
        return Object.assign({}, row, {
          ...
          liveOn: !row.sample,
          removeOn: !!row.sample,
          remove: this.memo('_rmb', id, () => () => this.askRemoveBank(id)),
          uploadOn: !row.sample,
          upload: this.memo('_upb', id, () => () => this.openStatementForAccount(id))
        });
      }),
      bankPreview: (() => {
        const row = s.bankPreview || {};
        return {
          name: row.name || '',
          initials: row.initials || 'QN',
          logo: row.logo || '/banks/qnb.png',
          iban: row.iban || '',
          balance: row.balance ? this.digits(row.balance) : ''
        };
      })(),
      overdueBanksOn: (s.overdueBankCount || 0) > 0,
      overdueBankBanner: this.t('ui.bank.overdueBanner', { count: s.overdueBankCount || 0 }),
      statementMonths: (() => {
        const data = s.statementMonths || { empty: true, months: [] };
        return {
          empty: !!data.empty,
          months: (data.months || []).map(month => ({
            id: month.id || month.label,
            label: this.display(month.label || ''),
            rows: (month.rows || []).map(row => Object.assign({}, row, {
              importedDate: this.display(row.importedDate || ''),
              rowLine: this.t('ui.bank.statementRows', { imported: this.digits(String(row.rowsImported ?? 0)), skipped: this.digits(String(row.rowsSkipped ?? 0)) }),
              periodLine: row.periodOn ? this.t('ui.bank.statementPeriod', { from: this.display(row.periodFrom), to: this.display(row.periodTo) }) : ''
            }))
          }))
        };
      })(),
      gateways: (() => {
        const g = s.gateways || { skipcash: {}, shopify: {} };
        const loc = (row) => !row ? row : Object.assign({}, row, {
          month: row.month ? this.digits(row.month) : row.month,
          settling: row.settling ? this.digits(row.settling) : row.settling,
          success: row.success ? this.digits(row.success) : row.success
        });
        return Object.assign({}, g, { skipcash: loc(g.skipcash), shopify: loc(g.shopify) });
      })(),
      scan: s.scan || {},
      periodFrom: toDateInput(s.periodFrom, -29),
      periodTo: toDateInput(s.periodTo, 0),
      f: Object.assign({ search: s.search }, FORM_BLANK, s.form),
      memberEdit: (() => {
      ...
      F: Object.assign({ search: (e) => { const v = e.target.value; this.setState({ search: v }); }, linkInvoice: this.onLinkInvoice, periodFrom: (e) => this.setState({ periodFrom: e.target.value }), periodTo: (e) => this.setState({ periodTo: e.target.value }), taxRegNumber: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { taxRegistrationNumber: e.target.value }) })), crNumber: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { crNumber: e.target.value }) })), profilePhone: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { phone: e.target.value }) })), profileEmail: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { email: e.target.value }) })), businessName: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { businessName: e.target.value }) })), legalEntity: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { legalEntity: e.target.value }) })), industry: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { industry: e.target.value }) })), profileAddress: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { address: e.target.value }) })), profileCurrency: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { currency: e.target.value }) })), profileBankName: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { bankName: e.target.value }) })), profileAccountName: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { accountName: e.target.value }) })), profileIban: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { iban: e.target.value }) })), profileAccountNumber: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { accountNumber: e.target.value }) })), profileSwiftCode: (e) => this.setState(st => ({ profile: Object.assign({}, st.profile, { swiftCode: e.target.value }) })), invDue: (e) => this.setState(st => ({ invDueError: '', form: Object.assign({}, st.form, { invDue: e.target.value }) })) }, this.forms(['empMethod', 'linkAmount', 'linkDesc', 'linkExpiry', 'linkClient', 'expAmount', 'expParty', 'invClient', 'invAmount', 'planName', 'planAmount', 'planInterval', 'planDesc', 'planCustomer', 'shopDomain', 'memberName', 'memberEmail', 'memberRole', 'empName', 'empRole', 'empSalary', 'scanVendor', 'scanAmount', 'scanDate', 'scanTag', 'scanTax', 'recEvery', 'recEnds', 'recClient', 'recAmount', 'cliName', 'cliEmail', 'cliPhone', 'cliAddress', 'stmtAccount', 'stmtBankName', 'stmtLabel', 'stmtPeriodFrom', 'stmtPeriodTo', 'bankDemoLoginId', 'bankDemoLoginPw', 'reminderDraft', 'aiChatDraft'])),
      h: {
        stop: (e) => e.stopPropagation(),
        back: () => this.setState({ detail: null }),
        openSearch: () => this.setState({ searchOpen: true, userMenu: false, moreOpen: false, aiChatOpen: false }),
        dismissFly: () => this.closeFly(true),
        ...
        openAiChat: () => this.setState({ aiChatOpen: true, userMenu: false, moreOpen: false, searchOpen: false, langMenu: false }),
        closeAiChat: () => this.setState({ aiChatOpen: false }),
        toggleAiChat: () => this.setState(s => ({ aiChatOpen: !s.aiChatOpen, userMenu: false, moreOpen: false, searchOpen: false, langMenu: false })),
        sendAiChatMessage: () => this.sendAiChatMessage(),
        sendAiChatSuggestion: (q) => this.sendAiChatMessage(q),
        railOpen: () => {
        ...
        toggleLang: () => this.setState(x => ({ langMenu: !x.langMenu, userMenu: false })),
        closeLang: () => this.setState({ langMenu: false }),
        branchMenu: this.go('reports', 'branches'),
        openHelp: this.openModal('help'),
        toggleUser: () => this.setState(x => ({ userMenu: !x.userMenu, langMenu: false })),
        closeUser: () => this.setState({ userMenu: false }),
        closeModal: () => { clearTimeout(this._scanT); this.setState({ modal: null, editEmp: null, editClient: null, editTag: null, scanBusy: false }); },
        ...
        newInvoice: this.go('invoicing', 'create'),
        addClient: this.openModal('client'),
        monthlySend: () => {
          const on = !this.state.toggles.monthlySend;
          this.setState(st => ({ toggles: Object.assign({}, st.toggles, { monthlySend: on }) }));
          this.toast(on ? this.t('ui.rep.monthlyToastOn') : this.t('ui.rep.monthlyToastOff'));
        },
        ...
        saveProfile: () => {
          ...
          if (typeof FlowStore !== 'undefined' && FlowStore.saveMerchantProfile) {
            FlowStore.saveMerchantProfile({
          ...
          this.toast(this.t('chrome.toast.saved'));
        },
        signOut: () => { this.setState({ userMenu: false, langMenu: false }); this.toast(this.t('chrome.toast.signedOut')); },
        syncNow: () => this.runZohoSync(),
        tallyExport: () => this.runTallyExport(),
        testTxn: () => this.toast('Sandbox test payment of QR 1.00 sent'),
        generatePayslips: () => this.generatePayslipsForPeriod(),
      ...
      detail: { on: !!d },
      modal: { on: !!s.modal, title: md[0], cta: md[1], hideFoot: s.modal === 'link', wide: s.modal === 'link', link: s.modal === 'link', scan: s.modal === 'scan', expense: s.modal === 'expense', invoice: s.modal === 'invoice', plan: s.modal === 'plan', member: s.modal === 'member', employee: s.modal === 'employee', client: s.modal === 'client', tag: s.modal === 'tag', help: s.modal === 'help', pageSettings: s.modal === 'pageSettings', receipts: s.modal === 'receipts', reset: s.modal === 'reset', removeBank: s.modal === 'removeBank', statement: s.modal === 'statement', bankDemo: s.modal === 'bankDemo', reminderPreview: s.modal === 'reminderPreview' },
      ...
      stmt: {
        accounts: [{ id: '__new__', label: this.t('ui.bank.newAccount') }].concat((s.banks || []).map(b => ({
          id: b.id,
          label: b.name || ((b.bank || '') + (b.label ? ', ' + b.label : ''))
        }))),
        banks: this.qatarBankPickerRows(s.form.stmtBankName, '_stmtb', 'stmtBankName'),
        creating: !s.form.stmtAccount || s.form.stmtAccount === '__new__',
        updatingOn: !!(s.form.stmtAccount && s.form.stmtAccount !== '__new__'),
        updatingLine: (() => {
          if (!s.form.stmtAccount || s.form.stmtAccount === '__new__') return '';
          const chosen = (s.banks || []).find(row => row.id === s.form.stmtAccount);
          if (!chosen) return '';
          return this.t('ui.bank.updatingAccount', {
            bank: chosen.bank || '',
            label: chosen.label || ''
          });
        })(),
        fileName: s.form.stmtFile || '',
        hasFile: !!s.form.stmtCsv,
        pickFile: this.memo('_stmt', 'pick', () => () => {
          const el = typeof document !== 'undefined' && document.getElementById('flow-stmt-file');
          if (el && el.click) el.click();
        }),
        onFile: this.memo('_stmt', 'chg', () => (e) => {
          const file = e && e.target && e.target.files && e.target.files[0];
          if (!file) {
            this.setState(st => ({ form: Object.assign({}, st.form, { stmtCsv: '', stmtFile: '' }) }));
            return;
          }
          const reader = new FileReader();
          reader.onload = () => {
            this.setState(st => ({ form: Object.assign({}, st.form, { stmtCsv: String(reader.result || ''), stmtFile: file.name }) }));
          };
          reader.onerror = () => this.toast(this.t('chrome.toast.stmtFailed'));
          reader.readAsText(file);
        })
      },
      bankDemoStep: s.bankDemoStep || 0,
      bankDemo: (() => {
        const step = s.bankDemoStep || 0;
        const banks = this.qatarBankPickerRows(s.form.bankDemoPick, '_bdemo', 'bankDemoPick');
        const picked = banks.find(row => row.on) || banks[0] || {};
        const preview = s.bankPreview || {};
        const otpRaw = String(s.form.bankDemoOtp || '');
        const pickedIds = Array.isArray(s.form.bankDemoAccountsPicked) ? s.form.bankDemoAccountsPicked : ['current', 'savings'];
        const mockAccounts = [
          { id: 'current', name: this.t('ui.bank.demoAccountLabel'), iban: 'QA•• •••• •••• 4821', amount: 128400 },
          { id: 'savings', name: this.t('ui.bank.demoSavingsLabel'), iban: 'QA•• •••• •••• 7710', amount: 42150 }
        ];
        return {
          step,
          step0: step === 0,
          ...
          banks,
          picked,
          otp: [0, 1, 2, 3, 4, 5].map(i => ({
            value: otpRaw[i] || '',
            set: this.memo('_dotp', String(i), () => (e) => {
              const ch = String(e && e.target && e.target.value || '').replace(/\D/g, '').slice(-1);
              this.setState(st => {
                const next = String(st.form.bankDemoOtp || '').split('');
                while (next.length < 6) next.push('');
                next[i] = ch;
                return { form: Object.assign({}, st.form, { bankDemoOtp: next.slice(0, 6).join('') }) };
              });
            })
          })),
          accounts: mockAccounts.map(row => ({
            id: row.id,
            name: row.name,
            note: row.iban + ' · ' + this.fmt(row.amount),
            on: pickedIds.indexOf(row.id) >= 0,
            toggle: this.memo('_bdacct', row.id, () => (e) => {
              const on = !!(e && e.target && e.target.checked);
              this.setState(st => {
                const cur = (Array.isArray(st.form.bankDemoAccountsPicked) ? st.form.bankDemoAccountsPicked : ['current', 'savings']).filter(id => id !== row.id);
                if (on) cur.push(row.id);
                return { form: Object.assign({}, st.form, { bankDemoAccountsPicked: cur }) };
              });
            })
          })),
          previewLabel: picked.bank ? (picked.bank + ', ' + this.t('ui.bank.demoAccountLabel')) : (preview.name || ''),
          previewLogo: picked.logo || preview.logo,
          previewInitials: picked.initials || preview.initials || 'QN',
          previewIban: preview.iban || '',
          previewBalance: preview.balance ? this.digits(preview.balance) : ''
        };
      })(),
      toast: { on: !!s.toast, msg: s.toast },
      tfs: [['day', this.t('overview.tf.day')], ['week', this.t('overview.tf.week')], ['month', this.t('overview.tf.month')]].map(([k, label]) => ({ label, on: s.tf === k, off: s.tf !== k, go: this.memo('_tf', k, () => () => this.setState({ tf: k })) })),
      reportsTfs: [['day', this.t('overview.tf.day')], ['week', this.t('overview.tf.week')], ['month', this.t('overview.tf.month')]].map(([k, label]) => ({ label, on: (s.reportsTf || 'month') === k, off: (s.reportsTf || 'month') !== k, go: this.memo('_rtf', k, () => () => this.setState({ reportsTf: k })) })),
      chart: (s.page === 'dashboard' && !d) ? this.chart() : CHART_VALS_IDLE, chartHover: this.chartHover, chartLeave: this.chartLeave,
      bal: {
        total: period.netText ? this.digits(period.netText) : this.fmt(period.net),
        pending: period.pendingText ? this.digits(period.pendingText) : this.fmt0(period.pending),
        pendingLine: this.t('overview.chart.pending', { amount: period.pendingText || ('QR ' + Math.abs(period.pending || 0).toLocaleString('en-US')) }),
        delta: this.display(period.netTrendText || '')
      },
      periodLabel: this.periodTitle(period.label || 'Last 30 days'),
      matchLabel: this.t('overview.period.thisMonth'),
      reportsLabel: '',
      balLabel: this.t('overview.chart.net'),
      match: {
        matched: String(autoN), totalItems: String(totalN), pctText: this.digits(matchPct + '%'), pctWidth: matchPct + '%',
        dash: ((matchPct / 100) * C).toFixed(1) + ' ' + C.toFixed(1),
        open: openN, hasOpen: openN > 0,
        openLabel: this.t('overview.kpi.open', { count: openN }),
        look: openN === 0 ? this.t('overview.match.lookZero') : openN === 1 ? this.t('overview.match.lookOne') : this.t('overview.match.lookMany', { count: openN }),
        of: this.t('overview.match.of', { matched: autoN, total: totalN })
      },
      auto: {
        open: !!s.autoOpen, shut: !s.autoOpen,
        count: String(autoN),
        pctText: matchPct + '%', pctW: matchPct + '%',
        line: this.t('overview.match.line', { matched: autoN, open: openN, count: openN }),
      ...
      recent: (() => {
        ...
        const cache = this._recentFeedCache;
        if (cache && cache.txns === txns && cache.locale === locale) return cache.output;
        const output = this.sortedTxns().slice(0, 6).map(t => this.row(t));
        this._recentFeedCache = { txns, locale, output };
        return output;
      })(),
      att: {
        any: attList.length > 0, none: attList.length === 0, count: String(attList.length), countText: this.digits(String(attList.length)),
        items: attList.map(a => {
          const rawNote = String(a.note || '');
          const sug = rawNote.match(/^Suggested match:\s*(.*)$/i);
          return {
            id: a.id, amt: this.fmt(a.amount), d: this.display(a.d), src: this.srcLabel(a.src),
            note: sug ? this.t('overview.att.suggested', { detail: this.display(sug[1]) }) : this.display(rawNote),
            conf: this.digits(typeof a.conf === 'number' ? a.conf + '%' : a.conf),
            confirm: this.resolveAtt(a.id, true), reject: this.resolveAtt(a.id, false)
          };
        })
      },
      quick,
      tagOptions: s.tags || [],
      expCat: this.catPicker(s, 'expTag'),
      scanCat: this.catPicker(s, 'scanTag'),
      tagPicks: this.tagTree(s).roots.map(t => ({ label: this.tagLabel(t), on: s.form.expTag === t || (s.tagParents || {})[s.form.expTag] === t, off: !(s.form.expTag === t || (s.tagParents || {})[s.form.expTag] === t), go: this.memo('_tp', t, () => () => this.setState(st => ({ form: Object.assign({}, st.form, { expTag: t }) }))) })),
      rolePicks: ['Owner', 'Accountant', 'Staff'].map(r => ({ label: this.t('pages.role.' + r.toLowerCase()), on: s.form.memberRole === r, off: s.form.memberRole !== r, go: this.memo('_rp', r, () => () => this.setState(st => ({ form: Object.assign({}, st.form, { memberRole: r }) }))) }))
    };
