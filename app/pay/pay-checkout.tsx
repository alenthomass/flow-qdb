...
const defaultFields: CheckoutField[] = [{ label: "Amount", kind: "price" }, { label: "Email", kind: "mail" }];
...
function PaymentForm({ slug, fields, amountMinor, payLabel, unavailable, outcome }: {
  ...
  const shownFields = unavailable && !processing ? [{ label: "Amount", kind: "price" }] : fields;
  async function pay() {
    ...
    if (Object.keys(nextErrors).length) {
      form.current?.querySelector<HTMLInputElement>("#pay-field-" + Object.keys(nextErrors)[0])?.focus();
      return;
    }
    busy.current = true;
    setState({ status: "processing" });
    const result = await submitPayment(slug, email, sandboxOutcome(outcome || null));
    setState(result);
    busy.current = false;
  }
  function submit(event: FormEvent) { event.preventDefault(); void pay(); }
  ...
  return <form ref={form} className="card" noValidate onSubmit={submit} aria-busy={processing}>
    <CardHeading>Payment details</CardHeading><div className="fields">
      {shownFields.map((field, index) => {
        ...
        const id = "pay-field-" + index;
        return <div className="field" key={id}>
          <label htmlFor={amount ? undefined : id}>{field.label || (amount ? "Amount" : "Field")}{!amount && (!field.optional || field.kind === "mail") && <span className="req">*</span>}</label>
          {amount ? <div className="box locked"><span className="chip">{currency === "QAR" ? "QR" : currency}</span><span>{amountText.replace(/^(QR|AED) /, "")}</span></div> : <div className="box"><input id={id} type={field.kind === "mail" ? "email" : field.kind === "phone" ? "tel" : "text"} placeholder={field.kind === "mail" ? "name@company.com" : ""} autoComplete={field.kind === "mail" ? "email" : field.kind === "phone" ? "tel" : "off"} required={!field.optional || field.kind === "mail"} disabled={processing} value={values[String(index)] || ""} aria-invalid={!!errors[String(index)]} aria-describedby={errors[String(index)] ? id + "-error" : undefined} onChange={event => {
            setValues(current => ({ ...current, [String(index)]: event.target.value }));
            setErrors(current => ({ ...current, [String(index)]: "" }));
          }} /></div>}
          {errors[String(index)] && <div id={id + "-error"} className="note err" role="alert">{errors[String(index)]}</div>}
        </div>;
      })}
    ...
    <div className="foot"><span style={{ flex: 1, minWidth: 0 }}><CardBrands /><span className="secured">Secured by SkipCash · SANDBOX</span></span>
      {(!unavailable || processing) && <button className="pay" type="button" disabled={processing} onClick={() => { void pay(); }}>{processing ? "Processing…" : payLabel + " " + amountText}</button>}
    </div>
  </form>;
}
...
export default function PayCheckout({ slug, outcome, receipt = false }: { slug: string; outcome?: string; receipt?: boolean }) {
  ...
  const emailFields = fields.some(field => field.kind === "mail") ? fields : [...fields, { label: "Email", kind: "mail" }];
  ...
  return <>
    <div className="grid"><MerchantCopy branding={branding} merchant={merchant} title={title} description={description} />
      {receiptTxn ? <SuccessCard result={{ status: "success", amountMinor: receiptTxn.amountMinor, merchant, reference: receiptTxn.id }} /> : <PaymentForm slug={slug} fields={emailFields} amountMinor={amountMinor} payLabel={page?.payLabel || "Pay"} unavailable={unavailable} outcome={outcome} />}
