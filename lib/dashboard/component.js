  ...
  renderVals() {
    ...
    const vals = {
      ...
      nv: this.needPage('invoicing', 'create') ? (() => {
        ...
        const persistLines = () => items.filter(x => x.desc || num(x.price)).map(x => ({
        ...
        const saveInvoice = (draft) => {
          ...
          if (disc > sub) { this.toast('Discount cannot exceed the subtotal'); return; }
          const dueRaw = iv.due == null
          ...
          const saved = this.persistCreatedInvoice({
            ...
            draft: !!draft,
            lines: persistLines(),
            partialPayment: !!iv.partialPayment,
            discountMinor: Math.round(disc * 100),
            attachments: files,
            clientAddress: String(iv.clientAddress || '').trim(),
            notes: String(iv.notes || '').trim(),
            reference: String(iv.reference || '').trim()
          });
          if (saved.error) {
        ...
        return {
          ...
          pdfBusy: !!s.invoicePdfBusy,
          downloadPdf: () => {
            if (!clientName || !sub) return;
            this.downloadCurrentInvoicePdf(nextNo, clientName);
          },
          printInvoice: () => {
            if (!clientName || !sub) return;
